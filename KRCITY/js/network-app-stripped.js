...
const R_DISTANT = 5;
const R_MAGNIFIED = 30; // For panning magnification near the viewport center

/**
 * Same base grid as KRCITY/index.html: `TILE` (60px) — see map constants in that file.
 * User nodes snap to TILE boundaries; collab block nodes use BLOCK_SIZE = TILE.
 * Macro building blocks in the game are 10×10 tiles (600px); not used for graph layout here.
 */
const KR_CITY_TILE_PX = 60;
const USER_GRID_PX = KR_CITY_TILE_PX;
...
const loadingScreen = document.getElementById('network-loading-screen'); // Get reference to loading screen (optional; stripped page has none)
...
const BLOCK_SIZE = KR_CITY_TILE_PX; // One game tile per collab block node (matches KRCITY map cells)
...
async function initNetwork() {
  ...
  simulation = d3.forceSimulation(nodes)
    ...
    .force("userGrid", (function createUserGridForce() {
      let simNodes;
      const strength = 0.55;
      function force(alpha) {
        for (const d of simNodes) {
          if (d.type === 'block') continue;
          const gx = Math.round(d.x / USER_GRID_PX) * USER_GRID_PX;
          const gy = Math.round(d.y / USER_GRID_PX) * USER_GRID_PX;
          d.vx += (gx - d.x) * strength * alpha;
          d.vy += (gy - d.y) * strength * alpha;
        }
      }
      force.initialize = (nodeList) => { simNodes = nodeList; };
      return force;
    })());
  ...
  // Faint world grid (same pitch as KRCITY TILE); behind graph, pans/zooms with content
  const gridPatternId = 'kr-network-tile-grid';
  svg.append('defs')
    .append('pattern')
    .attr('id', gridPatternId)
    .attr('width', KR_CITY_TILE_PX)
    .attr('height', KR_CITY_TILE_PX)
    .attr('patternUnits', 'userSpaceOnUse')
    .append('path')
    .attr('d', `M ${KR_CITY_TILE_PX} 0 L 0 0 0 ${KR_CITY_TILE_PX}`)
    .attr('fill', 'none')
    .attr('stroke', 'rgba(220, 222, 230, 0.09)')
    .attr('stroke-width', 1);
  ...
  // User–user links: orthogonal paths (only horizontal/vertical segments, 90° bend)
  function userToUserOrthogonalPath(d) {
    const sx = d.source.x;
    const sy = d.source.y;
    const tx = d.target.x;
    const ty = d.target.y;
  ...
  simulation.on("tick", () => {
    link.attr("d", userToUserOrthogonalPath);
...
function showProfileModal(user) {
  ...
  popupContent.querySelector('.popup-share-btn')?.remove();
  ...
  const joinAnchor = document.getElementById('buttonscrollanchor')?.closest('a');
  if (joinAnchor) joinAnchor.style.display = 'none';
  ...
  if (user.blocks && user.blocks.length > 0) {
    user.blocks.forEach((block, blockIdx) => {
      if (block.type === 'default' || !block.type) {
        ...
        const embedLink = safeHttpUrl(block.link) || '';
        blockContainer.innerHTML += `
          <div class="block block-embed" data-embed-link="${embedLink.replace(/"/g, '&quot;')}" style="flex-direction:column;align-items:stretch;">
            ${collabHtmlEmbed}
  ...
  // Make embed block text rows clickable to open the block link (read-only: no kebab share)
  const embedRows = blockContainer.querySelectorAll('.block.block-embed .block-text-kebab-row');
  embedRows.forEach(row => {
    const blockEl = row.closest('.block.block-embed');
    const link = blockEl && blockEl.getAttribute('data-embed-link');
    if (link) {
  ...
  if (!blockContainer.__clickableRowHandlersAdded) {
    blockContainer.addEventListener('click', (e) => {
      ...
      const link = row.dataset.link || block.getAttribute('data-embed-link');
      if (link) {
...
const closePopup = () => {
  ...
  setTimeout(() => {
    ...
    if (currentProfileIdInUrl || window.location.search.includes('link=')) {
      history.pushState(null, 'Network', 'indexstripped.html');
...
function updateNodeVisuals(selectedNodeId = null, nodesToMagnify = new Map()) {
  ...
  d3.selectAll('.node').each(function(d) {
    ...
    const isMagnified = isPanningMagnified;
  ...
  try {
    ...
    if (linkFromUrl && allUsers.length > 0) {
      ...
      if (targetUser) {
        ...
        history.replaceState(null, 'Network', 'indexstripped.html');
        updateNodeVisuals(null); // Reset visuals to default
      }
    } else {
