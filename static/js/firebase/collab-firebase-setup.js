...
async function updateUserData(userId, data) {
    const payload = { ...data };
    const titleChanging = payload.title !== undefined;

    if (titleChanging) {
        await claimDisplayNameImpl(db, userId, payload.title);
        delete payload.title;
        delete payload.titleSlug;
    }
...
async function getUserNotificationsForAdmin(userEmail) {
    if (!isAdmin()) throw new Error('Admin only');
    if (!userEmail) return [];
