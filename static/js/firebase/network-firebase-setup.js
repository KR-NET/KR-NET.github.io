...
import {
  ...
  generateGlobalBlockIdForOwner,
} from "./storage-paths.js";
import { invalidateUsersSessionCache, consumeUsersCacheDirtyFlag } from "./users-cache.js";
...
onAuthStateChanged(auth, async (user) => {
  ...
  console.log('[notifications] Firebase auth state resolved.', {
    hasUser: !!user,
    email: user && user.email ? user.email : null
  });
  currentUser = user;
...
async function ensureBlockHasGlobalId(owner, block, blocksArray) {
  const ownerEmail = typeof owner === 'string' ? owner : owner?.email;
  if (block.globalId) return block.globalId;
  const gid = generateGlobalBlockIdForOwner(owner, block.createdAt);
...
async function fetchAllUsers(options = {}) {
  ...
  try {
    ...
    debugLoad('firebase_fetch_all_users_cache_write_failed', {
      message: e && e.message ? e.message : String(e)
    });
  }
