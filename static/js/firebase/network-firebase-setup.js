...
async function fetchAllUsers() {
  console.log('[index.html / network-firebase-setup] Firestore: pulling users collection...');
  const snapshot = await getDocs(collection(db, 'users'));
  const users = snapshot.docs.map(doc => ({
    email: doc.id,
    ...doc.data()
  }));
  console.log('[index.html / network-firebase-setup] Firestore: loaded users', users.length);
  return users;
}

// Helper: Get user's full profile data
async function getUserProfile(email) {
  console.log('[index.html / network-firebase-setup] Firestore: pulling user profile', email);
  const docRef = doc(db, 'users', email);
  const docSnap = await getDoc(docRef);
  const result = docSnap.exists() ? { email: docSnap.id, ...docSnap.data() } : null;
  console.log('[index.html / network-firebase-setup] Firestore: loaded user profile', email, !!result);
  return result;
}
...
async function getUserNotifications(userEmail) {
    if (!userEmail) return [];
    console.log('[index.html / network-firebase-setup] Firestore: pulling notifications for', userEmail);
    ...
    console.log('[index.html / network-firebase-setup] Firestore: loaded notifications', notifications.length);
