...
import {
  ...
  generateLegacyGlobalBlockId,
  resolveGlobalBlockIdForDelete,
  resolveStorageAuthUid,
} from "./storage-paths.js";
import { invalidateUsersSessionCache } from "./users-cache.js";
import {
  claimDisplayName as claimDisplayNameImpl,
  checkDisplayNameAvailable as checkDisplayNameAvailableImpl,
} from "./title-registry.js";
import { validateDisplayTitle, slugifyTitle } from "../utils/slugify.js";
import { isBlockEmptyForGlobalPublish } from "../utils/block-limits.js";
...
// Always show Google's account picker (don't auto-use the last signed-in account)
...
onAuthStateChanged(auth, async (user) => {
  ...
  try {
    if (user) {
      ...
      // Keep authUid on the user doc for admin uploads and storage path resolution
      try {
        await setDoc(userDocRef, { authUid: user.uid }, { merge: true });
      } catch (e) {
        console.warn('Could not sync authUid to user profile', e);
      }
      ...
      if (bioEl) {
        ...
        if (bioCountEl) bioCountEl.textContent = "0/200";
      }
  ...
  unsubscribeFromUserData = onSnapshot(docRef, (docSnap) => {
    ...
    if (docSnap.exists()) {
      ...
      if (document.activeElement.id !== "bio") {
        const bioVal = (data.bio || "").slice(0, 200);
        const bioEl = document.getElementById("bio");
        if (bioEl) {
          bioEl.value = bioVal;
          const bioCountEl = document.getElementById("bio-count");
          if (bioCountEl) bioCountEl.textContent = `${bioVal.length}/200`;
        }
      }
...
async function saveUserData(email, profileData) {
  try {
    ...
    const titleChanging = profileData.title !== undefined;
    if (titleChanging) {
      await claimDisplayNameImpl(db, email, profileData.title);
      delete profileData.title;
      delete profileData.titleSlug;
    }
    ...
    if (titleChanging || profileData.avatar !== undefined) {
...
async function claimDisplayName(email, title) {
  return claimDisplayNameImpl(db, email, title);
}

async function checkDisplayNameAvailable(title, ownerEmail) {
  return checkDisplayNameAvailableImpl(db, title, ownerEmail);
}
...
function requireUserId(userDoc) {
  const data = userDoc ?? window.firebaseUtils?.currentUserDoc;
  const id = data?.id;
  if (id == null || id === '') {
    throw new Error('User profile id is required. Please reload the page.');
  }
  return String(id);
}

function requireAuthUid(authUser) {
  const user = authUser ?? window.firebaseUtils?.currentUser;
  const uid = user?.uid;
  if (!uid) {
...
async function upsertGlobalBlock(ownerEmail, block, ownerUserId) {
  if (!block || isBlockEmptyForGlobalPublish(block)) return null;
  const userId = ownerUserId != null ? String(ownerUserId) : requireUserId();
  const id = block.globalId || generateGlobalBlockId(userId, block.createdAt || Date.now());
  ...
  const payload = {
    ...
    slides: block.slides || [],
    collaborators: Array.isArray(block.collaborators) ? block.collaborators : [],
...
/**
 * Removes a block's globalId from a collaborator's collaborations array.
 * Used when the block owner removes a collaborator from the block.
 */
async function removeCollaborationFromUser(collaboratorEmail, blockGlobalId) {
  if (!collaboratorEmail || !blockGlobalId) return;
  try {
    const userRef = doc(db, 'users', collaboratorEmail);
    await updateDoc(userRef, {
      collaborations: arrayRemove(blockGlobalId)
    });
  } catch (err) {
    console.error('Error removing collaboration from user', err);
  }
}
...
window.firebaseUtils = {
  ...
  blockStoragePath,
  blockCarouselStoragePath,
  removeCollaborationFromUser,
  claimDisplayName,
  checkDisplayNameAvailable,
  validateDisplayTitle,
  slugifyTitle,
};
