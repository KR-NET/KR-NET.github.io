...
function initIntroGlobe() {
  ...
  // On mobile: show GIF instead of 3D globe, then fade out after same duration
  if (window.__isMobile) {
    overlay.classList.add('mobile-globe');
    ...
    // Match desktop animation duration (~1.5s) then fade out
    setTimeout(() => {
  ...
  if (!canvas) {
    console.error('Intro globe canvas not found');
    fallbackToMainContent();
    return;
  }
  ...
  // Load GeoJSON data (cached for welcome-globe)
  getGeoJson()
    .then(data => {
