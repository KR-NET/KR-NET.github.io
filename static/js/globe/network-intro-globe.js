...
import { getGeoJson } from "./geoJsonCache.js";
...
debugLoad('intro_globe_module_evaluated', {
...
function initNetworkIntroGlobe() {
  if (!canvas || !loadingScreen) {
    debugLoad('intro_globe_missing_elements', {
      hasCanvas: !!canvas,
  ...
  const isLoadingProfile = !!window.__krProfileSlugCandidate;
  const baseLoadingText = isLoadingProfile ? 'Loading profile' : 'Loading network';
  debugLoad('intro_globe_init_start', {
    isLoadingProfile,
  ...
  // Load GeoJSON data (cached with welcome globes via sessionStorage)
  getGeoJson()
    .then(data => {
      debugLoad('intro_globe_geojson_loaded', {
        hasFeatures: !!(data && data.features),
        featureCount: data && Array.isArray(data.features) ? data.features.length : undefined
      });
    ...
    .catch(error => {
      debugLoad('intro_globe_geojson_failed', {
  ...
  function enableInteraction() {
    isInteractive = true;
    debugLoad('intro_globe_enable_interaction');
...
export function hideNetworkLoadingScreen() {
  ...
  debugLoad('intro_globe_hide_requested', {
    hasCamera: !!camera,
  ...
  if (!camera || !renderer) {
    debugLoad('intro_globe_hide_using_fallback_path');
    completeLoadingScreenHide();
    return;
  }
  ...
  debugLoad('intro_globe_hide_start_zoom_phase', {
    zoomInStartZ
  });
}
...
function completeLoadingScreenHide() {
  debugLoad('intro_globe_complete_hide_start');
  loadingScreen.style.transition = 'opacity 0.3s ease-out';
  loadingScreen.style.opacity = '0';
  
  setTimeout(() => {
    loadingScreen.style.display = 'none';
    debugLoad('intro_globe_complete_hide_done');
...
debugLoad('intro_globe_hide_function_registered');

// Fallback when WebGL fails (e.g. mobile, low-power mode)
function fallbackHideLoadingScreen() {
  if (!loadingScreen) return;
  debugLoad('intro_globe_fallback_hide_start');
  if (loadingDotsInterval) {
    clearInterval(loadingDotsInterval);
    loadingDotsInterval = null;
  }
  ...
  setTimeout(() => {
    loadingScreen.style.display = 'none';
    debugLoad('intro_globe_fallback_hide_done');
  }, 300);
}
...
function init() {
  ...
  if (window.__isMobile) {
    debugLoad('intro_globe_init_skipped_for_mobile');
    return;
  }
  debugLoad('intro_globe_init_attempt_desktop');
  try {
    initNetworkIntroGlobe();
  } catch (e) {
    debugLoad('intro_globe_init_failed', {
