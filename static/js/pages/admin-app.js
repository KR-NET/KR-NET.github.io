import {
  avatarStoragePath,
  blockStoragePath,
  blockCarouselStoragePath,
  resolveStorageAuthUid,
} from '../firebase/storage-paths.js';
import {
  truncateBlockTitle,
  truncateBlockDesc,
...
import {
  computePendingConnectionRequestsForUser,
  computePendingCollaborationRequestsForUser,
  getCollaborationRequestBlockTitle,
  formatNotificationTimestamp,
  getNotificationCategoryIcon,
  buildSentLogIdSet,
} from '../utils/notification-preview.js';
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    let blockEditorMode = 'user'; // 'user' | 'global'
    let currentlyEditingGlobalBlock = null;
    let blockFormOriginalSnapshot = null;

    function getEditingUserAuthUid() {
        return resolveStorageAuthUid(currentlyEditingUser, window.collabFirebaseUtils?.currentUser);
    }

    function getBlockStorageAuthUid() {
        if (blockEditorMode === 'global' && currentlyEditingGlobalBlock?.owner) {
            const owner = allUsers.find(u => u.email === currentlyEditingGlobalBlock.owner);
            if (owner) return resolveStorageAuthUid(owner, window.collabFirebaseUtils?.currentUser);
        }
        return getEditingUserAuthUid();
    }
    ...
    const inboxPreviewUserSelect = document.getElementById('inbox-preview-user-select');
    const inboxPreviewRefreshBtn = document.getElementById('inbox-preview-refresh-btn');
    const inboxPreviewPanel = document.getElementById('inbox-preview-panel');

    let allBlocks = [];
    let inboxPreviewDebounceTimer = null;
    let inboxPreviewRequestId = 0;
    ...
    function populateUserSelect() {
        ...
        populateInboxPreviewUserSelect();
    }

    function populateInboxPreviewUserSelect() {
        if (!inboxPreviewUserSelect) return;
        const prev = inboxPreviewUserSelect.value;
        inboxPreviewUserSelect.innerHTML = '<option value="">Select a user…</option>' +
            allUsers.map(u => `<option value="${u.email}">${esc(u.title || u.email)} (${esc(u.email)})</option>`).join('');
        if (prev && allUsers.some(u => u.email === prev)) {
            inboxPreviewUserSelect.value = prev;
        }
    }

    function setInboxPreviewPlaceholder(message) {
        if (!inboxPreviewPanel) return;
        inboxPreviewPanel.innerHTML = `<p class="inbox-preview-placeholder">${esc(message)}</p>`;
    }

    function setInboxPreviewLoading() {
        if (!inboxPreviewPanel) return;
        inboxPreviewPanel.innerHTML = '<p class="inbox-preview-loading">Loading inbox…</p>';
    }

    function setInboxPreviewError(message) {
        if (!inboxPreviewPanel) return;
        inboxPreviewPanel.innerHTML = `<p class="inbox-preview-error">${esc(message)}</p>`;
    }

    function renderInboxPreviewSummary(connectionCount, collabCount, firestoreCount, unreadCount) {
        return `<div class="inbox-preview-summary">
            <span class="inbox-preview-stat"><strong>${connectionCount}</strong> connection</span>
            <span class="inbox-preview-stat"><strong>${collabCount}</strong> collaboration</span>
            <span class="inbox-preview-stat"><strong>${firestoreCount}</strong> broadcast</span>
            <span class="inbox-preview-stat"><strong>${unreadCount}</strong> unread</span>
        </div>`;
    }

    function renderInboxConnectionItems(users) {
        if (!users.length) return '<p class="inbox-preview-empty">No pending connection requests.</p>';
        return users.map(u => {
            const avatar = esc(u.avatar || 'static/img/default-avatar.png');
            const title = esc(u.title || u.email.split('@')[0]);
            const viewLink = buildProfileUrl(slugifyTitle(u.title || u.email.split('@')[0]));
            return `<div class="admin-inbox-item">
                <div class="admin-inbox-item-header">
                    <span class="admin-inbox-badge admin-inbox-badge--computed">Computed</span>
                    <span class="admin-inbox-badge">Connection</span>
                </div>
                <div class="admin-inbox-item-content">
                    <img src="${avatar}" alt="" class="admin-inbox-avatar">
                    <div>
                        <h4 class="admin-inbox-item-title">${title}</h4>
                        <p class="admin-inbox-item-body">wants to connect with this user</p>
                        <a href="${viewLink}" target="_blank" rel="noopener noreferrer" class="admin-inbox-link">View profile</a>
                        <div class="admin-inbox-item-meta">${esc(u.email)}</div>
                    </div>
                </div>
            </div>`;
        }).join('');
    ...
    function renderInboxCollaborationItems(items, targetEmail) {
        if (!items.length) return '<p class="inbox-preview-empty">No pending collaboration requests.</p>';
        return items.map(({ owner, block, globalId }) => {
            const avatar = esc(owner.avatar || 'static/img/default-avatar.png');
            const ownerTitle = esc(owner.title || owner.email.split('@')[0]);
            const blockTitle = esc(getCollaborationRequestBlockTitle(block, targetEmail));
            const viewLink = buildProfileUrl(slugifyTitle(owner.title || owner.email.split('@')[0]));
            const gidMeta = globalId ? esc(globalId) : '(no globalId)';
            return `<div class="admin-inbox-item">
                <div class="admin-inbox-item-header">
                    <span class="admin-inbox-badge admin-inbox-badge--computed">Computed</span>
                    <span class="admin-inbox-badge">Collaboration</span>
                </div>
                <div class="admin-inbox-item-content">
                    <img src="${avatar}" alt="" class="admin-inbox-avatar">
                    <div>
                        <h4 class="admin-inbox-item-title">${ownerTitle}</h4>
                        <p class="admin-inbox-item-body">wants to add as collaborator on “${blockTitle}”</p>
                        <a href="${viewLink}" target="_blank" rel="noopener noreferrer" class="admin-inbox-link">View profile</a>
                        <div class="admin-inbox-item-meta">${esc(owner.email)} · ${gidMeta}</div>
                    </div>
                </div>
            </div>`;
        }).join('');
    ...
    function renderInboxFirestoreItems(notifications, sentLogIds) {
        if (!notifications.length) return '<p class="inbox-preview-empty">No Firestore inbox notifications.</p>';
        return notifications.map(n => {
            const isUnread = !n.read;
            const date = esc(formatNotificationTimestamp(n.timestamp));
            const category = esc(n.category || 'Update');
            const icon = esc(getNotificationCategoryIcon(n.category));
            const imageHtml = n.imageUrl
                ? `<img src="${esc(n.imageUrl)}" alt="" class="admin-inbox-thumb">`
                : '';
            const linkHtml = n.link
                ? `<a href="${esc(n.link)}" target="_blank" rel="noopener noreferrer" class="admin-inbox-link">View more</a>`
                : '';
            const staleBadge = n.logId && !sentLogIds.has(n.logId)
                ? '<span class="admin-inbox-badge admin-inbox-badge--stale">Stale log</span>'
                : '';
            const unreadBadge = isUnread
                ? '<span class="admin-inbox-badge admin-inbox-badge--unread">Unread</span>'
                : '';
            return `<div class="admin-inbox-item${isUnread ? ' unread' : ''}">
                <div class="admin-inbox-item-header">
                    <span class="admin-inbox-badge admin-inbox-badge--firestore">Firestore</span>
                    ${unreadBadge}
                    ${staleBadge}
                    <img src="${icon}" alt="" style="width:18px;height:18px;">
                    <span style="color:#aaa;font-size:0.85em;">${category}</span>
                    <span style="color:#666;font-size:0.8em;margin-left:auto;">${date}</span>
                </div>
                <div class="admin-inbox-item-content">
                    <div style="flex:1;">
                        <h4 class="admin-inbox-item-title">${esc(n.title || 'No title')}</h4>
                        <p class="admin-inbox-item-body">${esc(n.body || '')}</p>
                        ${linkHtml}
                        <div class="admin-inbox-item-meta">id: ${esc(n.id)}${n.logId ? ` · logId: ${esc(n.logId)}` : ''}</div>
                    </div>
                    ${imageHtml}
                </div>
            </div>`;
    ...
    function renderUserInboxPreview(email, profile, firestoreNotifications, pendingConnections, pendingCollabs) {
        const unreadCount = firestoreNotifications.filter(n => !n.read).length;
        const sentLogIds = buildSentLogIdSet(allSentNotifications);
        const displayName = esc(profile?.title || email);

        inboxPreviewPanel.innerHTML =
            renderInboxPreviewSummary(
                pendingConnections.length,
                pendingCollabs.length,
                firestoreNotifications.length,
                unreadCount
            ) +
            `<p class="admin-section-note" style="margin-top:0;">Previewing inbox for <strong>${displayName}</strong> (${esc(email)}) — read-only.</p>` +
            `<div class="inbox-preview-subsection">
                <h3>Connection requests (${pendingConnections.length})</h3>
                <p class="subsection-note">Derived from <code>users.connections</code> (not stored in notifications).</p>
                ${renderInboxConnectionItems(pendingConnections)}
            </div>` +
            `<div class="inbox-preview-subsection">
                <h3>Collaboration requests (${pendingCollabs.length})</h3>
                <p class="subsection-note">Derived from block <code>collaborators</code> arrays.</p>
                ${renderInboxCollaborationItems(pendingCollabs, email)}
            </div>` +
            `<div class="inbox-preview-subsection">
                <h3>Broadcast notifications (${firestoreNotifications.length})</h3>
                <p class="subsection-note">From <code>users/${esc(email)}/notifications</code>.</p>
                ${renderInboxFirestoreItems(firestoreNotifications, sentLogIds)}
            </div>`;
    }

    async function loadUserInboxPreview(email) {
        if (!email) {
            setInboxPreviewPlaceholder('Select a user to preview their inbox.');
            if (inboxPreviewRefreshBtn) inboxPreviewRefreshBtn.disabled = true;
            return;
        }
        if (inboxPreviewRefreshBtn) inboxPreviewRefreshBtn.disabled = false;

        const requestId = ++inboxPreviewRequestId;
        setInboxPreviewLoading();

        try {
            const cached = allUsers.find(u => u.email === email);
            const profile = (await getUserProfile(email)) || cached || { email };
            const firestoreNotifications = await getUserNotificationsForAdmin(email);
            if (requestId !== inboxPreviewRequestId) return;

            const pendingConnections = computePendingConnectionRequestsForUser(email, allUsers, profile);
            const pendingCollabs = computePendingCollaborationRequestsForUser(email, allUsers, profile);
            renderUserInboxPreview(email, profile, firestoreNotifications, pendingConnections, pendingCollabs);
        } catch (e) {
            if (requestId !== inboxPreviewRequestId) return;
            setInboxPreviewError(e.message || 'Failed to load inbox.');
            console.error('[admin] loadUserInboxPreview', e);
        }
    }

    function scheduleInboxPreviewLoad(email) {
        clearTimeout(inboxPreviewDebounceTimer);
        inboxPreviewDebounceTimer = setTimeout(() => loadUserInboxPreview(email), 300);
    }
    ...
    if (inboxPreviewUserSelect) {
        inboxPreviewUserSelect.addEventListener('change', () => {
            scheduleInboxPreviewLoad(inboxPreviewUserSelect.value.trim());
        });
    }
    if (inboxPreviewRefreshBtn) {
        inboxPreviewRefreshBtn.addEventListener('click', () => {
            const email = inboxPreviewUserSelect?.value?.trim();
            if (email) loadUserInboxPreview(email);
        });
    }
    ...
    document.addEventListener('click', async (e) => {
        ...
        if (button.matches('.admin-delete-btn')) {
            ...
            openGlobalBlockEditor(allBlocks.find(b => b.id === id));
        }
    });
        ...
        return `
            ...
            <div class="user-info-card">
                ...
                <div class="user-info-grid">
                    ...
                    <div class="user-info-item">
                        <span class="user-info-label">Collaborations</span>
                        <span class="user-info-value">${(user.collaborations || []).length} accepted</span>
                    </div>
                    <div class="user-info-item">
                        <span class="user-info-label">Dismissed Requests</span>
                        <span class="user-info-value">${(user.dismissedCollaborationRequests || []).length}</span>
                    </div>
                </div>
            </div>
            ...
            <form id="admin-edit-user-form">
                ...
                <div class="form-field admin-collab-section">
                    <label>User Collaborations</label>
                    <p style="font-size: 12px; color: #666; margin: 0 0 6px 0;">Block globalIds this user has accepted (comma-separated). These blocks appear on their network.</p>
                    <textarea id="edit-user-collaborations" class="input-feild" rows="2" placeholder="e.g. user@email.com_blockId1, user@email.com_blockId2">${(user.collaborations || []).join(', ')}</textarea>
                </div>
                <div class="form-field admin-collab-section">
                    <label>Dismissed Collaboration Requests</label>
                    <p style="font-size: 12px; color: #666; margin: 0 0 6px 0;">Block globalIds this user has declined (comma-separated).</p>
                    <textarea id="edit-user-dismissed-collaborations" class="input-feild" rows="2" placeholder="e.g. user@email.com_blockId1">${(user.dismissedCollaborationRequests || []).join(', ')}</textarea>
                </div>
                <div class="form-field">
                    <label>Blocks</label>
    ...
    async function handleUserUpdate(e) {
        ...
        const collabRaw = (form.querySelector('#edit-user-collaborations')?.value || '').trim();
        const collaborations = collabRaw ? collabRaw.split(/[,\s]+/).map(s => s.trim()).filter(Boolean) : [];
        const dismissedRaw = (form.querySelector('#edit-user-dismissed-collaborations')?.value || '').trim();
        const dismissedCollaborationRequests = dismissedRaw ? dismissedRaw.split(/[,\s]+/).map(s => s.trim()).filter(Boolean) : [];
        ...
        const updatedData = {
            ...
            connections,
            collaborations,
            dismissedCollaborationRequests,
    ...
    function renderAdminUserBlocks() {
        ...
        currentlyEditingUserBlocks.forEach((block, index) => {
            ...
            const collabList = Array.isArray(block.collaborators) && block.collaborators.length ? block.collaborators.join(', ') : '';
            const collabHtml = collabList ? `<div class="admin-block-collabs" style="font-size:12px;color:#666;margin-top:4px;">Collaborators: ${esc(collabList)}</div>` : '';
            ...
            blockDiv.innerHTML = `
                <div class="block-content">
                    ...
                    <div class="block-text">${blockHTML}${collabHtml}</div>
                </div>
    ...
    function showGlobalAdminPanel(show) {
        const panel = document.getElementById('admin-block-global-panel');
        if (panel) panel.style.display = show ? 'block' : 'none';
    }

    function parseCreatedAtToDate(createdAt) {
    ...
    function setCreatedAtInput(createdAt) {
        const input = document.getElementById('admin-block-created-at');
        if (!input) return;
        const d = parseCreatedAtToDate(createdAt);
        if (d) {
    ...
    function formatCommentTime(timestamp) {
        const d = parseCreatedAtToDate(timestamp);
        if (!d) return '';
        return d.toLocaleString(undefined, { dateStyle: 'medium', timeStyle: 'short' });
    }

    function populateGlobalMetaPanel(block) {
        const meta = document.getElementById('admin-block-global-meta');
        if (!meta || !block) return;
        const type = block.type || 'default';
        const up = block.upvotes ?? block.up ?? 0;
        const down = block.downvotes ?? block.down ?? 0;
        const score = block.score ?? (up - down);
        meta.innerHTML = `
            <div class="admin-meta-item"><span class="admin-meta-label">Global ID</span><span class="admin-meta-value">${esc(block.id)}</span></div>
            <div class="admin-meta-item"><span class="admin-meta-label">Owner</span><span class="admin-meta-value">${esc(block.owner || '—')}</span></div>
            <div class="admin-meta-item"><span class="admin-meta-label">Type</span><span class="admin-meta-value">${esc(type)}</span></div>
            <div class="admin-meta-item"><span class="admin-meta-label">Votes</span><span class="admin-meta-value">▲ ${up} · ▼ ${down} · Score ${score}</span></div>
        `;
    }

    async function renderGlobalBlockComments(blockId) {
        const list = document.getElementById('admin-block-comments-list');
        if (!list) return;
        list.innerHTML = '<p class="admin-no-comments">Loading comments…</p>';
        const commentsSnap = await window.collabFirebaseUtils.getBlockComments(blockId).catch(() => null);
        if (!commentsSnap) {
            list.innerHTML = '<p class="admin-no-comments">Error loading comments.</p>';
            return;
        }
        if (commentsSnap.empty) {
            list.innerHTML = '<p class="admin-no-comments">No comments yet.</p>';
            return;
        }
        list.innerHTML = commentsSnap.docs.map(ds => {
            const c = ds.data();
            const authorEmail = c.author || '';
            const ownerUser = allUsers.find(u => u.email === authorEmail);
            const authorTitle = c.authorTitle || ownerUser?.title || authorEmail || 'Unknown';
            return `<div class="admin-block-comment" data-id="${ds.id}">
                <div class="admin-block-comment-body">
                    <div class="admin-block-comment-author">${esc(authorTitle)}${authorEmail ? `<span class="admin-block-comment-email">${esc(authorEmail)}</span>` : ''}</div>
                    <p class="admin-block-comment-text">${esc(c.text || '')}</p>
                    <div class="admin-block-comment-time">${esc(formatCommentTime(c.timestamp))}</div>
                </div>
                <button type="button" class="admin-block-comment-delete gb-del-comment-btn" data-cid="${ds.id}">Delete</button>
            </div>`;
        }).join('');
    }
    ...
    function getBlockCollaboratorEmails(block) {
        ...
        if (block.type === 'carousel' && Array.isArray(block.slides)) {
            ...
            block.slides.forEach(slide => {
                (Array.isArray(slide.collaborators) ? slide.collaborators : []).forEach(e => emails.add(e));
            });
            return [...emails];
        }
        return Array.isArray(block.collaborators) ? block.collaborators : [];
    }

    function getBlockFormSnapshot() {
        return JSON.stringify({
            ...
            icon: document.getElementById('admin-block-img')?.dataset.icon || '',
            collaborators: document.getElementById('admin-block-collaborators')?.value || '',
            createdAt: document.getElementById('admin-block-created-at')?.value || '',
            embed: currentEmbed,
    ...
    function hasUnsavedBlockChanges() {
        if (!blockFormOriginalSnapshot) return false;
        return getBlockFormSnapshot() !== blockFormOriginalSnapshot;
    }

    function closeBlockModal(force = false) {
        const modal = document.getElementById('admin-edit-block-modal');
        if (!modal) return;
        if (!force && hasUnsavedBlockChanges()) {
            if (!confirm('You have unsaved changes. Discard them?')) return;
        }
        modal.style.display = 'none';
        blockEditorMode = 'user';
        currentlyEditingGlobalBlock = null;
        blockFormOriginalSnapshot = null;
        editingBlockIndex = null;
    }

    function populateBlockFormFromData(block) {
        currentEmbed = null;
        if (!block) return;

        const collabInput = document.getElementById('admin-block-collaborators');
        if (collabInput) {
            const collabs = Array.isArray(block.collaborators) ? block.collaborators : [];
            collabInput.value = collabs.join(', ');
        }

        if (block.type === 'carousel') {
            setBlockType('carousel', false);
            carouselSlides = JSON.parse(JSON.stringify(block.slides || []));
            carouselSlides.forEach(s => { if (!s.imgFile) s.imgFile = null; });
            while (carouselSlides.length < 2) {
    ...
    function openGlobalBlockEditor(block) {
        if (!block) return;
        const modal = document.getElementById('admin-edit-block-modal');
        blockEditorMode = 'global';
        currentlyEditingGlobalBlock = { ...block, id: block.id };
        currentlyEditingUser = allUsers.find(u => u.email === block.owner) || null;
        editingBlockIndex = null;

        document.getElementById('admin-modal-title').textContent = 'Edit Global Block';
        showGlobalAdminPanel(true);
        populateGlobalMetaPanel(block);
        setCreatedAtInput(block.createdAt);
        renderGlobalBlockComments(block.id);
        populateBlockFormFromData(block);

        blockFormOriginalSnapshot = getBlockFormSnapshot();
    ...
    function initializeBlockEditor() {
        ...
        // Close button
        const closeBtn = modal.querySelector('.close-button');
        closeBtn.onclick = () => closeBlockModal();

        // Cancel button
        document.getElementById('admin-cancel-block').onclick = () => closeBlockModal();
        ...
        const adminBlockTitle = document.getElementById('admin-block-title');
        const adminBlockDesc = document.getElementById('admin-block-description');
        initBlockTitleField(adminBlockTitle, document.getElementById('admin-block-title-count'), renderBlockLivePreview);
        initBlockDescField(adminBlockDesc, document.getElementById('admin-block-description-count'), renderBlockLivePreview);
        ...
        const commentsList = document.getElementById('admin-block-comments-list');
        if (commentsList) {
            commentsList.addEventListener('click', async (e) => {
                const btn = e.target.closest('.gb-del-comment-btn');
                if (!btn || blockEditorMode !== 'global' || !currentlyEditingGlobalBlock) return;
                const cid = btn.dataset.cid;
                if (!confirm('Delete this comment?')) return;
                btn.disabled = true;
                await window.collabFirebaseUtils.deleteBlockComment(currentlyEditingGlobalBlock.id, cid).catch(err => alert(err.message));
                await renderGlobalBlockComments(currentlyEditingGlobalBlock.id);
            });
        }
    }
    ...
    function setBlockType(type, isNew) {
        ...
        if (blockEditorMode === 'global') {
            const typeMeta = document.querySelector('#admin-block-global-meta .admin-meta-item:nth-child(3) .admin-meta-value');
            if (typeMeta) typeMeta.textContent = type;
        }
    }
    ...
    function renderBlockLivePreview() {
        ...
        if (currentBlockType === 'embed' && currentEmbed) {
            ...
            } else if (blockEditorMode === 'global' && currentlyEditingGlobalBlock?.icon) {
                imgSrc = currentlyEditingGlobalBlock.icon;
    ...
    function createCarouselSlideField(slide, idx) {
        ...
        div.innerHTML = `
            ...
            <div class='carousel-slide-details' style='display: ${detailsVisible};'>
                ...
                <input type='text' placeholder='Slide Title' value="${esc(slide.title || '')}" class='input-feild collab-input-field carousel-slide-title' maxlength="100">
                ...
                <textarea placeholder='Slide Description' class='input-feild collab-input-field carousel-slide-desc' rows="3" maxlength="400">${esc(slide.desc || '')}</textarea>
    ...
    async function onSaveBlock() {
        const { uploadImage, deleteImage, updateGlobalBlock, updateUserData, removeCollaborationFromUser } = window.collabFirebaseUtils;
        ...
        try {
            let newBlockData = {};
            const storageUid = getBlockStorageAuthUid();
            ...
            if (currentBlockType === 'carousel') {
                ...
                for (let i = 0; i < carouselSlides.length; i++) {
                    ...
                    let iconUrl = slide.icon || '';

                    if (slide.imgFile) {
                        const file = slide.imgFile;
                        const path = blockCarouselStoragePath(storageUid, i, `${Date.now()}_${file.name}`);
                        iconUrl = await uploadImage(file, path);
                    } else if (imgInput && imgInput.dataset.fetchedUrl) {
                        iconUrl = imgInput.dataset.fetchedUrl;
                    }
                    
                    if (slide.title || slide.link) {
                        slidesToSave.push({
                            title: truncateBlockTitle(slide.title),
                            desc: truncateBlockDesc(slide.desc),
                            link: normalizeUrl(slide.link),
                ...
                newBlockData = {
                    ...
                    title: truncateBlockTitle(document.getElementById('admin-block-title').value),
                    desc: truncateBlockDesc(document.getElementById('admin-block-description').value),
                    link: normalizeUrl(document.getElementById('admin-block-link').value),
                    icon: DEFAULT_ICON
                };
            }
            if (currentBlockType !== 'carousel' && currentBlockType !== 'embed') {
                const imgInput = document.getElementById('admin-block-img');
                let iconUrl = imgInput.dataset.icon || '';
                if (blockEditorMode === 'global' && currentlyEditingGlobalBlock?.icon) {
                    iconUrl = iconUrl || currentlyEditingGlobalBlock.icon;
                } else if (editingBlockIndex !== null) {
                    iconUrl = iconUrl || currentlyEditingUserBlocks[editingBlockIndex].icon || '';
                }
                
                if (imgInput.files[0]) {
                    const file = imgInput.files[0];
                    const path = blockStoragePath(storageUid, `${Date.now()}_${file.name}`);
                    iconUrl = await uploadImage(file, path);
                } else if (imgInput.dataset.fetchedUrl) {
                    iconUrl = imgInput.dataset.fetchedUrl;
                }
            ...
            if (collabInput) {
                ...
                newBlockData.collaborators = raw ? raw.split(/[,\s]+/).map(s => s.trim()).filter(Boolean) : [];
            }

            if (blockEditorMode === 'global') {
                await saveGlobalBlock(newBlockData, { uploadImage, deleteImage, updateGlobalBlock, updateUserData, removeCollaborationFromUser });
                return;
            }

            if (editingBlockIndex !== null) {
                const existing = currentlyEditingUserBlocks[editingBlockIndex];
                if (existing.globalId) newBlockData.globalId = existing.globalId;
                if (existing.createdAt) newBlockData.createdAt = existing.createdAt;
            }
    ...
    async function saveGlobalBlock(newBlockData, utils) {
        const { deleteImage, updateGlobalBlock, updateUserData, removeCollaborationFromUser } = utils;
        const oldBlock = currentlyEditingGlobalBlock;
        const blockId = oldBlock.id;
        const globalId = blockId;

        newBlockData.globalId = globalId;

        const oldCollabs = getBlockCollaboratorEmails(oldBlock);
        const newCollabs = newBlockData.collaborators || [];
        const removedCollabs = oldCollabs.filter(o => !newCollabs.includes(o));
        for (const email of removedCollabs) {
            await removeCollaborationFromUser(email, globalId);
        }

        if (oldBlock.type === 'carousel' && newBlockData.type !== 'carousel') {
            for (const slide of (oldBlock.slides || [])) {
                if (slide.icon && !slide.icon.includes('default-icon')) {
                    await deleteImage(slide.icon).catch(() => {});
                }
            }
        }

        let createdAt = oldBlock.createdAt;
        const createdInput = document.getElementById('admin-block-created-at');
        if (createdInput?.value) {
            createdAt = new Date(createdInput.value).toISOString();
        ...
        newBlockData.createdAt = createdAt;
        newBlockData.link = normalizeUrl(newBlockData.link || '');

        const up = oldBlock.upvotes ?? oldBlock.up ?? 0;
        const down = oldBlock.downvotes ?? oldBlock.down ?? 0;

        const payload = {
            owner: oldBlock.owner,
            title: newBlockData.title || '',
            desc: newBlockData.desc || '',
            link: newBlockData.link || '',
            icon: newBlockData.icon || DEFAULT_ICON,
            type: newBlockData.type || 'default',
            slides: newBlockData.type === 'carousel' ? (newBlockData.slides || []) : [],
            collaborators: newCollabs,
            provider: newBlockData.type === 'embed' ? (newBlockData.provider || null) : null,
            embedUrl: newBlockData.type === 'embed' ? (newBlockData.embedUrl || null) : null,
            createdAt,
            upvotes: up,
            downvotes: down,
            score: up - down,
        ...
        await updateGlobalBlock(blockId, payload);

        if (oldBlock.owner) {
            const owner = allUsers.find(u => u.email === oldBlock.owner);
            const blocks = [...(owner?.blocks || [])];
            const idx = blocks.findIndex(b => b.globalId === globalId);
            const ownerBlock = {
                ...newBlockData,
                globalId,
                createdAt,
            ...
            if (idx >= 0) {
                blocks[idx] = ownerBlock;
            } else {
                blocks.push(ownerBlock);
            }
            await updateUserData(oldBlock.owner, { blocks });
            if (owner) owner.blocks = blocks;
        }
                ...
                img.onload = () => {
                    ...
                    modal.querySelector("#crop-confirm").onclick = async () => {
                        ...
                        try {
                            ...
                            const avatarPath = avatarStoragePath(getEditingUserAuthUid(), `${Date.now()}.jpg`);
                            const avatarUrl = await window.collabFirebaseUtils.uploadImage(compressedFile, avatarPath);
