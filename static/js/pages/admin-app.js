        ...
        try {
            ...
            if (currentBlockType === 'carousel') {
                ...
                for (let i = 0; i < carouselSlides.length; i++) {
                    ...
                    if (slide.imgFile) { // Priority 1: New manual upload for this slide
                        ...
                        const path = blockCarouselStoragePath(getEditingUserAuthUid(), i, `${Date.now()}_${file.name}`);
            ...
            if (currentBlockType !== 'carousel' && currentBlockType !== 'embed') {
                ...
                if (imgInput.files[0]) { // Priority 1: New manual upload
                    ...
                    const path = blockStoragePath(getEditingUserAuthUid(), `${Date.now()}_${file.name}`);
                    iconUrl = await uploadImage(file, path);
            ...
            // Collaborators: parse comma-separated emails
            const collabInput = document.getElementById('admin-block-collaborators');
            if (collabInput) {
            ...
            // Preserve globalId and createdAt when editing
            if (editingBlockIndex !== null) {
