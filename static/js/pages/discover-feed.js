...
async function loadAcceptedCollaboratorsAndRenderChips(block, chipsRow) {
  const collab = Array.isArray(block.collaborators) ? block.collaborators : [];
  if (!block.globalId || collab.length === 0) return;
  try {
    const accepted = [];
    for (const email of collab) {
      let data = window.__userCache?.[email];
      if (!data) {
        const snap = await getDoc(doc(db, 'users', email));
        data = snap.exists() ? snap.data() : null;
        if (!window.__userCache) window.__userCache = {};
        if (data) window.__userCache[email] = data;
      }
      if (data && (Array.isArray(data.collaborations) ? data.collaborations : []).includes(block.globalId)) {
        accepted.push({ email, ...data });
      }
    }
    ...
    const chips = accepted.map(u => {
      ...
      return `<div class="collaborator-chip discover-collaborator-chip" role="button" tabindex="0" onclick="event.stopPropagation(); window.open('${profileLink.replace(/'/g, "\\'")}', '_blank')" onkeydown="if(event.key==='Enter'||event.key===' '){event.preventDefault();event.stopPropagation();window.open('${profileLink.replace(/'/g, "\\'")}','_blank')}"><img src="${avatar}" alt="${sanitizeText(title)}" class="collaborator-chip-avatar"><span class="collaborator-chip-info"><span class="collaborator-chip-name">${sanitizeText(title)}</span>${bioShort ? `<span class="collaborator-chip-bio"> | ${sanitizeText(bioShort)}</span>` : ''}</span><img src="static/img/Share.png" alt="View" class="collaborator-chip-link" width="12" height="12"></div>`;
    }).join('');
    chipsRow.innerHTML = `<div class="block-collaborators discover-block-collaborators" onclick="event.stopPropagation()"><span class="block-collaborators-label">COLLABORATION BETWEEN</span><div class="block-collaborators-chips">${chips}</div></div>`;
  } catch (e) {
    console.error('discover collaborator chips', e);
  }
}
...
function renderBlockCard(block, container) {
  ...
  headerTopRow.className = 'discover-post-header-top';
  ...
  headerTopRow.appendChild(avatar);
  headerTopRow.appendChild(titleWrap);
  header.appendChild(headerTopRow);

  const chipsRow = document.createElement('div');
  chipsRow.className = 'discover-post-header-chips';
  header.appendChild(chipsRow);

  // Make the top row clickable to open the author's profile
  headerTopRow.style.cursor = 'pointer';
  headerTopRow.onclick = () => {
    ...
    window.location.href = buildProfileUrl(slug);
  };
  ...
  try {
    ...
    if (me && me === block.owner) {
      ...
      headerTopRow.appendChild(actionsWrap);
    }
  } catch(_){}

  // Load accepted collaborators and render chips at bottom of header
  loadAcceptedCollaboratorsAndRenderChips(block, chipsRow);
...
export async function initDiscoverFeed(containerEl) {
  if (!db || !window.networkFirebaseUtils?.db) {
