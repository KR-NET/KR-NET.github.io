...
import { consumeUsersCacheDirtyFlag } from "../firebase/users-cache.js";
import { slugifyTitle } from "../utils/slugify.js";
import {
  buildProfileUrl,
  parseProfileSlugFromLocation,
  profilePathForSlug,
  isLikelyProfileDeepLink,
  parseProfileSlugFromUrlString
} from "../utils/profile-url.js";
...
let nodes = [];
let links = [];
let collabLinks = []; // Block-to-collaborator links (orthogonal paths)
let collabBlocks = []; // Block nodes data for rendering
let simulation;
let zoomBehavior;
let nodeDegrees = {};
let userLinkSelection = null;
let collabPathSelection = null;
let nodeSelection = null;
let networkGraphRefreshInFlight = false;
...
debugLoad('network_app_module_evaluated', {
  isInIframe,
  isMobile: typeof window !== 'undefined' ? window.__isMobile : undefined
});
...
let stickyViewCommunityTimeout = null;
let stickyButtonShown = false; // Track if button has been shown (by scroll or timeout)

const POPUP_SCROLL_THRESHOLD = 20; // px - show button & gradient when user scrolls past this
const POPUP_SCROLL_GRADIENT_MAX = 250; // px - scroll distance for "full strength" gradient
const DARK_OVERLAY = 'rgba(0, 0, 0, 0.6)';
const IRIS_OUT_DURATION_MS = 450; // Match popup-iris-out animation in styles.css
let _popupScrollRaf = null;
let isPopupClosing = false;

function handlePopupScroll() {
  if (!scrollableModalContentElement || !popupContainerElement || !stickyOverlayButtonElement) return;
  const scrollTop = scrollableModalContentElement.scrollTop;
  const scrollDown = scrollTop > POPUP_SCROLL_THRESHOLD;

  if (scrollDown) {
    // Show button immediately if not yet shown
    if (!stickyButtonShown) {
      stickyButtonShown = true;
      if (stickyViewCommunityTimeout) {
        clearTimeout(stickyViewCommunityTimeout);
        stickyViewCommunityTimeout = null;
      }
      stickyOverlayButtonElement.style.display = 'flex';
      stickyOverlayButtonElement.classList.add('sticky-view-community-fly-up');
    }
  }

  // Throttle background updates to avoid jitter; always use gradient format for smooth transition
  if (_popupScrollRaf) cancelAnimationFrame(_popupScrollRaf);
  _popupScrollRaf = requestAnimationFrame(() => {
    _popupScrollRaf = null;
    if (!popupContainerElement) return;
    const scrollBeyond = Math.max(0, scrollTop - POPUP_SCROLL_THRESHOLD);
    let progress = Math.min(1, scrollBeyond / POPUP_SCROLL_GRADIENT_MAX);
    progress = progress * progress; // ease-in: gentler start, avoids abrupt change at threshold
    const opacity = 0.02 + progress * 1.68; // 0.02 (barely visible) → 0.7 (strong)
    const extent = 8 + progress * 52; // 8% (just bottom) → 60% (extends up)
    // Always use gradient format so transition is smooth (no switch between solid/gradient)
    const bottomColor = progress < 0.001 ? DARK_OVERLAY : `rgba(0,0,255,${opacity})`;
    popupContainerElement.style.background = `linear-gradient(to top, ${bottomColor} 0%, ${DARK_OVERLAY} ${Math.max(1, extent)}%)`;
  });
}
...
let pendingCollaborationRequests = [];
...
let hasInitializedNetworkData = false;
let shouldRetryNotificationsAfterAuth = false;
let notificationRetryInFlight = false;

const initialLoadingContextIsProfile = isLikelyProfileDeepLink();
...
const BLOCK_SIZE = 48; // Rectangular block node size (same for all)
const BLOCK_RADIUS = Math.hypot(BLOCK_SIZE, BLOCK_SIZE) / 2; // For collision
const visionMediaState = new Map();
const nodeRadiusCache = new Map();
let visionCircleScreenState = null;
let visionCircleSimState = null;
let visionLayoutTick = 0;
const VISION_MEDIA_IMAGE_SIZE = 25;
const VISION_MEDIA_VISIBLE_MAX = 5;
const VISION_MEDIA_VISIBLE_ITEMS = 4;
const VISION_MEDIA_NODE_GAP = 14;
const VISION_MEDIA_ROW_STEP = 22;
const VISION_MEDIA_HALO_PADDING = 1;
const VISION_MEDIA_ANGLE_SAMPLES = 24;
const VISION_MEDIA_HYSTERESIS = 0.22;
const VISION_LAYOUT_RECALC_INTERVAL = 8;
const VISION_LAYOUT_MOVE_THRESHOLD = 10;
const VISION_FADE_UPDATE_INTERVAL = 3;
...
let collabLinksLayer = null; // SVG <g> layer for collaborative block links (orthogonal paths)

function computeNodeDegrees(nodeList, linkList) {
  const degrees = {};
  nodeList.forEach(node => { degrees[node.id] = 0; });
...
function buildNetworkGraphData(usersSnapshot) {
  const visibleUsers = usersSnapshot.filter(u => typeof u.bio === 'string' && u.bio.trim().length > 0);
  const visibleEmails = new Set(visibleUsers.map(u => u.email));

  const userNodes = visibleUsers.map(user => ({
    id: user.email,
  ...
  visibleUsers.forEach(user => {
    if (user.connections) {
      user.connections.forEach(targetEmail => {
        if (!visibleEmails.has(targetEmail)) return;
        ...
        if (targetUser && Array.isArray(targetUser.connections) && targetUser.connections.includes(user.email)) {
          userLinks.push({ source: user.email, target: targetEmail });
        }
      });
    }
  });

  const nextCollabBlocks = [];
  const nextCollabLinks = [];
  const sharedCollabPairs = new Set();

  visibleUsers.forEach(user => {
    (user.blocks || []).forEach(block => {
      if (!block.globalId) return;

      if (block.type === 'carousel' && Array.isArray(block.slides)) {
        block.slides.forEach((slide, slideIdx) => {
          const slideCollaborators = Array.isArray(slide?.collaborators) ? slide.collaborators : [];
          if (slideCollaborators.length === 0) return;

          const accepted = getAcceptedCollaboratorsForEmails(slideCollaborators, block.globalId, usersSnapshot);
          if (accepted.length === 0) return;

          const allParties = [user.email, ...accepted.map(u => u.email)];
          const allVisible = allParties.every(email => visibleEmails.has(email));
          if (!allVisible) return;

          for (let i = 0; i < allParties.length; i++) {
            for (let j = i + 1; j < allParties.length; j++) {
              sharedCollabPairs.add(getUserPairKey(allParties[i], allParties[j]));
            }
          }

          const slideNodeId = `${block.globalId}__slide_${slideIdx}`;
          nextCollabBlocks.push({
            id: slideNodeId,
            type: 'block',
            title: getCarouselSlideNodeTitle(block, slide, slideIdx),
            icon: slide.icon || block.icon || 'static/img/default-avatar.png',
            owner: user.email,
            block: block,
            acceptedEmails: allParties
          });

          allParties.forEach(targetEmail => {
            nextCollabLinks.push({
              source: slideNodeId,
              target: targetEmail,
              isCollab: true
            });
      ...
      if (!Array.isArray(block.collaborators) || block.collaborators.length === 0) return;
      const accepted = getAcceptedCollaboratorsForEmails(block.collaborators, block.globalId, usersSnapshot);
      if (accepted.length === 0) return;
      const allParties = [user.email, ...accepted.map(u => u.email)];
      const allVisible = allParties.every(email => visibleEmails.has(email));
      if (!allVisible) return;
      ...
      nextCollabBlocks.push({
        ...
        type: 'block',
        title: block.title || '',
        icon: block.icon || 'static/img/default-avatar.png',
        owner: user.email,
        block: block,
        acceptedEmails: allParties
      });
  ...
  userLinks = userLinks.map(link => ({
    ...link,
    hasSharedCollab: sharedCollabPairs.has(getUserPairKey(link.source, link.target))
  }));

  return {
    collabBlocks: nextCollabBlocks,
    collabLinks: nextCollabLinks,
    nodes: [...nextCollabBlocks, ...userNodes],
    links: [...userLinks, ...nextCollabLinks]
  };
}

function configureSimulationLinkForce(sim, linkData) {
  sim.force('link', d3.forceLink(linkData)
    .id(d => d.id)
    .distance(link => {
      if (link.isCollab) return 35;
      if (link.hasSharedCollab) return 35;
      const sourceDegree = nodeDegrees[link.source.id] || 1;
      const targetDegree = nodeDegrees[link.target.id] || 1;
      if (sourceDegree === 1 || targetDegree === 1) return 40;
      return Math.min(50 + (sourceDegree + targetDegree) * 2, 120);
    })
    .strength(link => {
      if (link.isCollab) return 0.85;
      if (link.hasSharedCollab) return 0.7;
      const sourceDegree = nodeDegrees[link.source.id] || 1;
      const targetDegree = nodeDegrees[link.target.id] || 1;
      if (sourceDegree === 1 || targetDegree === 1) return 2.0;
      if (sourceDegree >= 3 || targetDegree >= 3) return 0.15;
      return 0.4;
    })
  );
}

function orthogonalCollabPath(d) {
  const src = d.source;
  const tgt = d.target;
  const bx = src.x;
  const by = src.y;
  ...
  if (useHorizontalEdge) {
    ex = bx + (dx >= 0 ? half : -half);
    ey = by;
    return `M ${ex} ${ey} H ${tx} V ${ty}`;
  }

  ex = bx;
  ey = by + (dy >= 0 ? half : -half);
  return `M ${ex} ${ey} V ${ty} H ${tx}`;
}

function networkLinkKey(d) {
  const sourceId = typeof d.source === 'object' ? d.source.id : d.source;
  const targetId = typeof d.target === 'object' ? d.target.id : d.target;
  return sourceId < targetId ? `${sourceId}__${targetId}` : `${targetId}__${sourceId}`;
}

function networkCollabLinkKey(d) {
  const sourceId = typeof d.source === 'object' ? d.source.id : d.source;
  const targetId = typeof d.target === 'object' ? d.target.id : d.target;
...
function preserveNodePositions(previousNodes, nextNodes) {
  ...
  nextNodes.forEach(n => {
    ...
    if (prev) {
      ...
      if (prev.vx != null) n.vx = prev.vx;
      if (prev.vy != null) n.vy = prev.vy;
      return;
    }
    if (n.type !== 'block' || !Array.isArray(n.acceptedEmails)) return;
    const partyNodes = n.acceptedEmails
      .map(email => nextNodes.find(node => node.id === email))
      .filter(p => p && p.x != null && p.y != null);
    if (partyNodes.length === 0) return;
    n.x = partyNodes.reduce((sum, p) => sum + p.x, 0) / partyNodes.length;
    n.y = partyNodes.reduce((sum, p) => sum + p.y, 0) / partyNodes.length;
  });
}

function networkNodeDragBehavior() {
  return d3.drag()
...
function appendNetworkNodeEnter(enterSelection) {
  const blockEnter = enterSelection.filter(d => d.type === 'block');
  const half = BLOCK_SIZE / 2;
  blockEnter.append('rect')
    ...
    .style('fill', '#2a2a2a')
    .style('stroke', '#555');
  blockEnter.append('image')
    .attr('href', d => d.icon)
    .attr('x', -half + 4)
    .attr('y', -half + 4)
    .attr('width', BLOCK_SIZE - 8)
    .attr('height', BLOCK_SIZE - 8)
    .attr('preserveAspectRatio', 'xMidYMid slice')
    .attr('class', 'block-node-image');

  const userEnter = enterSelection.filter(d => d.type !== 'block');
  userEnter.append('defs')
    .append('clipPath')
    .attr('id', d => `clip-${d.id.replace(/[^a-zA-Z0-9]/g, '-')}`)
    .append('circle')
    .attr('r', 0);
  userEnter.append('circle')
...
async function handleNetworkNodeClick(event, d) {
  const profileId = d.type === 'block' ? d.owner : d.id;
  updateNodeVisuals(profileId);
  ...
  d3.selectAll('.block-node').classed('node-glow', false);
  ...
  playNodeClickSound();
  triggerHaptic();
  const userProfile = await window.networkFirebaseUtils.getUserProfile(profileId);
  if (userProfile) {
    if (userProfile.title || userProfile.email) {
      const slug = slugifyTitle(userProfile.title || (userProfile.email || '').split('@')[0]);
      history.pushState({ profileSlug: slug }, `Profile ${userProfile.title || userProfile.email}`, profilePathForSlug(slug));
...
function syncNetworkGraphDom() {
  if (!simulation || !userLinkSelection || !collabPathSelection || !nodeSelection) return;

  const userLinksOnly = links.filter(l => !l.isCollab);

  userLinkSelection = userLinkSelection
    .data(userLinksOnly, networkLinkKey);
  userLinkSelection.exit().remove();
  userLinkSelection = userLinkSelection.enter()
    .append('line')
    .attr('class', d => d.hasSharedCollab ? 'link link-has-collab' : 'link')
    .merge(userLinkSelection);

  collabPathSelection = collabPathSelection
    .data(collabLinks, networkCollabLinkKey);
  collabPathSelection.exit().remove();
  collabPathSelection = collabPathSelection.enter()
    .append('path')
  ...
  const nodeEnter = nodeSelection.enter()
    .append('g')
    .attr('class', d => (d.type === 'block' ? 'block-node' : 'node'))
    .call(networkNodeDragBehavior());
  appendNetworkNodeEnter(nodeEnter);
  nodeSelection = nodeEnter.merge(nodeSelection);
  nodeSelection.on('click', handleNetworkNodeClick);

  nodeSelection.filter(d => d.type === 'block').select('image.block-node-image')
    .attr('href', d => d.icon);
  nodeSelection.filter(d => d.type !== 'block').select('image.node-avatar')
    .attr('href', d => d.avatar || 'static/img/default-avatar.png');
  nodeSelection.filter(d => d.type !== 'block').select('text.node-title')
    .text(d => d.title);
}

async function refreshNetworkGraphAfterSocialAccept() {
  if (!hasInitializedNetworkData || !simulation || !window.networkFirebaseUtils) return;
  if (networkGraphRefreshInFlight) return;

  networkGraphRefreshInFlight = true;
  try {
    window.networkFirebaseUtils.invalidateUsersSessionCache();
    const freshUsers = await window.networkFirebaseUtils.fetchAllUsers({ bypassCache: true });
    if (!Array.isArray(freshUsers) || freshUsers.length === 0) return;

    const previousNodes = nodes;
    allUsers = freshUsers;

    const graph = buildNetworkGraphData(allUsers);
    collabBlocks = graph.collabBlocks;
    collabLinks = graph.collabLinks;
    nodes = graph.nodes;
    links = graph.links;

    preserveNodePositions(previousNodes, nodes);
    nodeDegrees = computeNodeDegrees(nodes, links);

    simulation.nodes(nodes);
    configureSimulationLinkForce(simulation, links);

    syncNetworkGraphDom();
    simulation.alpha(0.35).restart();
    try {
      for (let i = 0; i < 3; i++) simulation.tick();
    } catch (_) { /* ignore tick errors during warm-up */ }

    updatePracticeLinks();
    updateNodeVisuals(activeClickedNodeId);
    updateLocateButtonVisibility();
  } catch (err) {
    console.warn('[refreshNetworkGraphAfterSocialAccept]', err);
  } finally {
    networkGraphRefreshInFlight = false;
  }
}

// Initialize the network visualization
async function initNetwork() {
  debugLoad('network_app_init_start', {
    hasFirebaseUtils: !!window.networkFirebaseUtils,
    hasHideFn: typeof window.hideNetworkLoadingScreen === 'function'
  });
  try {
    // Fetch all users from Firestore
    debugLoad('network_app_fetch_all_users_start');
    allUsers = await window.networkFirebaseUtils.fetchAllUsers();
    debugLoad('network_app_fetch_all_users_success', {
      count: allUsers.length
    });
    console.log('[initNetwork] allUsers loaded:', allUsers.length, '| sample blocks with collaborators:', allUsers.flatMap(u => (u.blocks || []).filter(b => (b.collaborators || []).length > 0).map(b => ({ owner: u.email, collaborators: b.collaborators, globalId: b.globalId }))));
  } catch (e) {
    allUsers = [];
    debugLoad('network_app_fetch_all_users_failed', {
      message: e && e.message ? e.message : String(e)
    });
    console.error('[initNetwork] fetchAllUsers failed:', e);
  }

  // --- Early profile display when /slug: show popup immediately, don't wait for graph ---
  const linkFromUrl = parseProfileSlugFromLocation();
  debugLoad('network_app_deeplink_check', {
    linkFromUrl,
  ...
  if (linkFromUrl && allUsers.length > 0) {
    ...
    if (targetUser) {
      debugLoad('network_app_deeplink_target_found', {
        email: targetUser.email,
        title: targetUser.title || null
      });
      activeClickedNodeId = targetUser.email;
      currentProfileIdInUrl = linkFromUrl;
      window.__krProfileOpenedOnLoad = true; // Hints wait until popup closes
      // Hide loading, reveal UI, show profile modal immediately
      debugLoad('network_app_deeplink_request_hide_loader', {
        hasHideFn: typeof window.hideNetworkLoadingScreen === 'function'
      });
      if (window.hideNetworkLoadingScreen) window.hideNetworkLoadingScreen();
      if (!window.__uiRevealed) {
        window.__uiRevealed = true;
        revealUIStaggered();
      }
      debugLoad('network_app_deeplink_show_profile_modal');
      showProfileModal(targetUser);
    } else {
      debugLoad('network_app_deeplink_target_not_found', {
        linkFromUrl
      });
  ...
  // Hide the loading screen with globe animation (if not already hidden by early profile)
  debugLoad('network_app_request_hide_loader_post_init', {
    hasHideFn: typeof window.hideNetworkLoadingScreen === 'function'
  });
  ...
  nodes = graph.nodes;
  links = graph.links;
  debugLoad('network_app_visible_users_built', {
    visibleUsers: nodes.filter(n => n.type !== 'block').length,
    totalUsers: allUsers.length
  });

  nodeDegrees = computeNodeDegrees(nodes, links);
  debugLoad('network_app_nodes_links_built', {
    nodes: nodes.length,
    links: links.length,
    collabBlocks: collabBlocks.length,
    collabLinks: collabLinks.length
  });
  ...
  simulation = d3.forceSimulation(nodes)
    .force("charge", d3.forceManyBody().strength(d => {
        ...
        if (degree === 1) {
            return -10; // Weaker repulsion for single-connection nodes to keep them close.
        }
        return -700; // Stronger repulsion for all other nodes to spread them out.
    }))
    .force("center", d3.forceCenter(
      window.innerWidth / 2, 
      fixedHeaderHeight + (window.innerHeight - fixedHeaderHeight) / 2 // Center in the visible part of the SVG
    ))
    .force("collide", d3.forceCollide(d => (d.type === 'block' ? BLOCK_RADIUS : R_SELECTED + 2)))
    .force("titleSpread", function titleSpreadForce(alpha) {
      const HORIZ_THRESHOLD = 90;
      const VERT_THRESHOLD = 34;
      const STRENGTH = 2.12;
      for (let i = 0; i < nodes.length; i++) {
        for (let j = i + 1; j < nodes.length; j++) {
          ...
          const dx = a.x - b.x;
          const dy = a.y - b.y;
          if (Math.abs(dx) > HORIZ_THRESHOLD) continue;
          if (Math.abs(dy) > VERT_THRESHOLD) continue;
          const dir = (a.id < b.id) ? 1 : -1;
          const sep = (VERT_THRESHOLD - Math.abs(dy)) * STRENGTH * alpha;
          a.vy += -dir * sep;
          b.vy += dir * sep;
        }
      }
  ...
  configureSimulationLinkForce(simulation, links);
  debugLoad('network_app_simulation_created', {
    nodes: nodes.length,
    links: links.length
  });
  ...
  if (isInIframe) {
    ...
    function isNodeOrUIElement(target) {
      ...
      // Check if it's a node (has .node or .block-node class or is child of one)
      if (target.classList && (target.classList.contains('node') || target.classList.contains('block-node'))) return true;
      if (target.closest && (target.closest('.node') || target.closest('.block-node'))) return true;
      // Check if it's a circle, image, text, or rect that's part of a node
      if (target.tagName === 'circle' || target.tagName === 'image' || target.tagName === 'text' || target.tagName === 'rect') return true;
      // Check if it's a link
      if (target.tagName === 'line' || target.tagName === 'path') return true;
      return false;
    }
  ...
  // Collab links layer: orthogonal paths (behind user links)
  collabLinksLayer = container.append("g").attr("class", "collab-links-layer");
  collabPathSelection = collabLinksLayer
    .selectAll("path")
    .data(collabLinks)
    .join("path")
    .attr("class", "collab-link")
  ...
  // Create the links (user links only - straight lines)
  const userLinks = links.filter(l => !l.isCollab);
  userLinkSelection = container.append("g")
    .attr("class", "user-links-layer")
    .selectAll("line")
    .data(userLinks)
    .join("line")
    .attr("class", d => d.hasSharedCollab ? "link link-has-collab" : "link");
  ...
  nodeSelection = container.append("g")
    ...
    .attr("class", d => (d.type === 'block' ? 'block-node' : 'node'))
    .call(networkNodeDragBehavior());
  appendNetworkNodeEnter(nodeSelection);
  ...
  simulation.on("tick", () => {
    ...
    collabPathSelection.attr("d", orthogonalCollabPath);
    ...
    if (isVisionModeActive) {
      updateVisionMediaLayouts(false);
      if (visionLayoutTick % VISION_FADE_UPDATE_INTERVAL === 0) {
        applyVisionContextFade();
      }
    }
  });
...
// Helper: get collaborators who have accepted (in allUsers and have globalId in their collaborations)
function getAcceptedCollaborators(block) {
  ...
  return collab
    ...
    .filter(Boolean)
    .filter(u => (Array.isArray(u.collaborations) ? u.collaborations : []).includes(block.globalId));
}

function getAcceptedCollaboratorsForEmails(emails, globalId, usersSnapshot = allUsers) {
  if (!globalId || !Array.isArray(emails) || emails.length === 0) return [];
  return [...new Set(emails)]
    .map(email => usersSnapshot.find(u => u.email === email))
    .filter(Boolean)
    .filter(u => (Array.isArray(u.collaborations) ? u.collaborations : []).includes(globalId));
}

// Helper: render collaborator chips HTML for a block (uses div+onclick to avoid nested <a> inside block links)
function renderCollaboratorsHtml(block) {
  return renderCollaboratorsHtmlForEmails(
    Array.isArray(block?.collaborators) ? block.collaborators : [],
    block?.globalId
  );
}

function renderCollaboratorsHtmlForEmails(emails, globalId) {
  const accepted = getAcceptedCollaboratorsForEmails(emails, globalId);
  if (accepted.length === 0) return '';
  const chips = accepted.map(u => {
    const avatar = u.avatar || 'static/img/default-avatar.png';
    const title = u.title || u.email.split('@')[0];
    const bioShort = (u.bio || '').slice(0, 30) + ((u.bio || '').length > 30 ? '...' : '');
    const slug = slugifyTitle(title);
    const profileLink = buildProfileUrl(slug);
    return `
      <div class="collaborator-chip" role="button" tabindex="0" data-href="${profileLink}" onclick="event.stopPropagation(); event.preventDefault(); window.open('${profileLink.replace(/'/g, "\\'")}', '_blank')" onkeydown="if(event.key==='Enter'||event.key===' ') { event.preventDefault(); window.open('${profileLink.replace(/'/g, "\\'")}', '_blank'); }">
        <img src="${avatar}" alt="${title}" class="collaborator-chip-avatar">
        <span class="collaborator-chip-info">
          <span class="collaborator-chip-name">${sanitizeText(title)}</span>
          ${bioShort ? `<span class="collaborator-chip-bio"> | ${sanitizeText(bioShort)}</span>` : ''}
        </span>
        <img src="static/img/Share.png" alt="View" class="collaborator-chip-link" width="12" height="12">
      </div>
    `;
  }).join('');
  return `
    <div class="block-collaborators" onclick="event.stopPropagation()">
      <span class="block-collaborators-label">COLLABORATION BETWEEN</span>
      <div class="block-collaborators-chips">${chips}</div>
    </div>
  `;
}

function getCollaborationRequestBlockTitle(block, collaboratorEmail = '') {
...
function getCarouselSlideNodeTitle(block, slide, slideIdx = 0) {
...
function showProfileModal(user) {
  debugLoad('network_app_show_profile_modal', {
  ...
  // Assign to higher-scope variables for access in closePopup
  ...
  stickyButtonShown = false;
  // Ensure sticky button has content and click handler; show with 2.3s delayed fly-up (or immediately on scroll)
  if (stickyOverlayButtonElement) {
    ...
    stickyOverlayButtonElement.classList.remove('sticky-view-community-fly-up');
    // Clear any previous timeout
    if (stickyViewCommunityTimeout) clearTimeout(stickyViewCommunityTimeout);
    stickyViewCommunityTimeout = setTimeout(() => {
      if (!stickyButtonShown) {
        stickyButtonShown = true;
        stickyOverlayButtonElement.style.display = 'flex';
        stickyOverlayButtonElement.classList.add('sticky-view-community-fly-up');
  ...
  // Scroll listener: show button immediately + blue gradient when user scrolls down
  if (scrollableModalContentElement && !scrollableModalContentElement._popupScrollListener) {
    scrollableModalContentElement._popupScrollListener = handlePopupScroll;
    scrollableModalContentElement.addEventListener('scroll', handlePopupScroll);
  }
  handlePopupScroll(); // Initial check (e.g. if popup opens pre-scrolled)
  
  // Hide clear filters button when popup is open
  clearFiltersBtn.style.display = 'none';
  
  // Remove any legacy top view-community button (no longer used)
  popupContent.querySelector('.view-community-btn-div')?.remove();
  ...
  if (user.blocks && user.blocks.length > 0) {
    user.blocks.forEach((block, blockIdx) => {
      if (block.type === 'default' || !block.type) {
        ...
        const collabHtml = renderCollaboratorsHtml(block);
        ...
        const collabHtmlLarge = renderCollaboratorsHtml(block);
        blockContainer.innerHTML += `
        ...
        const collabHtmlEmbed = renderCollaboratorsHtml(block);
        blockContainer.innerHTML += `
        ...
        blockContainer.innerHTML += `
          <div class="carousel-blocks-wrapper">
            <div class="carousel-blocks">
            ${block.slides.map((slide, slideIdx) => `
              <div class="carousel-slide-card">
                ${renderCollaboratorsHtmlForEmails(Array.isArray(slide.collaborators) ? slide.collaborators : [], block.globalId)}
  ...
  // Reset scroll to top so each profile opens at the top (not where the previous one left off).
  // Must run after popup is visible; double rAF ensures layout has settled.
...
// Close profile modal (with iris-out animation)
const closePopup = () => {
  if (isPopupClosing) {
    debugLoad('network_app_close_popup_ignored_already_closing');
    return;
  }
  if (!popupContainerElement) {
    debugLoad('network_app_close_popup_ignored_missing_container');
    return;
  }

  debugLoad('network_app_close_popup_start', {
    currentProfileIdInUrl,
    activeClickedNodeId,
  ...
  if (_popupScrollRaf) {
    cancelAnimationFrame(_popupScrollRaf);
    _popupScrollRaf = null;
  }

  // Hide sticky button immediately so it doesn't linger during iris-out
  if (stickyOverlayButtonElement) {
    stickyOverlayButtonElement.style.display = 'none';
    stickyOverlayButtonElement.classList.remove('sticky-view-community-fly-up');
  }

  // Remove popup scroll listener
  if (scrollableModalContentElement && scrollableModalContentElement._popupScrollListener) {
    scrollableModalContentElement.removeEventListener('scroll', scrollableModalContentElement._popupScrollListener);
    scrollableModalContentElement._popupScrollListener = null;
  }

  // Trigger iris-out: circle shrinks toward center
  popupContainerElement.classList.add('iris-out-closing');

  const container = popupContainerElement;
  setTimeout(() => {
    container.style.display = 'none';
    container.classList.remove('iris-out-closing');
    container.style.background = DARK_OVERLAY;
    delete container.dataset._scrollDebug;
    ...
    if (currentProfileIdInUrl || parseProfileSlugFromLocation()) {
      history.pushState(null, "Network", "/");
      currentProfileIdInUrl = null;
      debugLoad('network_app_close_popup_cleared_deeplink_url');
    }
    ...
    // Signal hints to start (when user arrived via ?link= and closed profile popup)
    if (window.__krProfileOpenedOnLoad) {
      window.__krProfileOpenedOnLoad = false;
      window.dispatchEvent(new Event('kr_profile_popup_closed'));
    }
    debugLoad('network_app_close_popup_complete', {
      activeClickedNodeId,
      searchTerm,
      selectedPracticesCount: selectedPractices ? selectedPractices.length : 0
    });
  }, IRIS_OUT_DURATION_MS);
};
...
if (popupCloseButton) {
  popupCloseButton.addEventListener('click', (event) => {
    ...
    try { playBackButtonSound(); } catch (_) {}
    closePopup();
  });
}

const popupContainerRoot = document.getElementById('popup-container');
const popupBackground = document.getElementById('popup-background');
let pendingOutsidePopupPointer = null;

function isPopupVisible() {
  return !!(popupContainerElement && popupContainerElement.style.display !== 'none');
}

function isInteractivePopupOverlayTarget(target) {
  if (!(target instanceof Element)) return false;
  if (target.closest('.popup')) return true;
  if (target.closest('#sticky-view-community-button')) return true;
  return false;
}

function resetPendingOutsidePopupPointer() {
  pendingOutsidePopupPointer = null;
}

function handlePopupOutsidePointerDown(event) {
  if (!isPopupVisible() || isPopupClosing) {
    resetPendingOutsidePopupPointer();
    return;
  }
  if (isInteractivePopupOverlayTarget(event.target)) {
    resetPendingOutsidePopupPointer();
    return;
  }

  pendingOutsidePopupPointer = {
...
function handlePopupOutsidePointerUp(event) {
  if (!pendingOutsidePopupPointer || !isPopupVisible() || isPopupClosing) {
    resetPendingOutsidePopupPointer();
    return;
  }

  if (event.pointerId !== pendingOutsidePopupPointer.pointerId) {
    return;
  }

  const deltaX = event.clientX - pendingOutsidePopupPointer.clientX;
  const deltaY = event.clientY - pendingOutsidePopupPointer.clientY;
  const movedTooFar = Math.hypot(deltaX, deltaY) > 10;
  const endedInsidePopupUi = isInteractivePopupOverlayTarget(event.target);

  resetPendingOutsidePopupPointer();

  if (movedTooFar || endedInsidePopupUi) {
    return;
  }
...
if (popupContainerRoot) {
  popupContainerRoot.addEventListener('pointerdown', handlePopupOutsidePointerDown);
  popupContainerRoot.addEventListener('pointerup', handlePopupOutsidePointerUp);
  popupContainerRoot.addEventListener('pointercancel', resetPendingOutsidePopupPointer);
}
...
function updateNodeVisuals(selectedNodeId = null, nodesToMagnify = new Map()) {
  ...
  d3.selectAll('.node').each(function(d) {
    ...
    nodeRadiusCache.set(nodeId, targetRadius);
  ...
  // --- Update Block Node & Collab Link Visuals ---
  d3.selectAll('.block-node').each(function(d) {
    const el = d3.select(this);
    let isHighlighted = false;
    let isDimmed = false;
    if (selectedNodeId) {
      isHighlighted = (d.acceptedEmails || []).includes(selectedNodeId);
      isDimmed = !isHighlighted;
    }
    el.classed('highlighted-node', isHighlighted);
    el.classed('dimmed', isDimmed);
    el.transition().duration(300).style('opacity', isDimmed ? 0.6 : 1);
  });
  d3.selectAll('.collab-link').each(function(l) {
    const el = d3.select(this);
    const isHighlighted = selectedNodeId && (l.source.id === selectedNodeId || l.target.id === selectedNodeId);
    const isDimmed = selectedNodeId && !isHighlighted;
    el.classed('highlighted-link', isHighlighted);
    el.classed('dimmed', isDimmed);
    el.transition().duration(300).style('stroke-opacity', isDimmed ? 0.25 : 0.6);
  });
}
...
let hasBootstrappedNetworkPage = false;

async function refreshNetworkUserAvatars() {
  if (!hasInitializedNetworkData || !window.networkFirebaseUtils) return;
  if (!consumeUsersCacheDirtyFlag()) return;
  try {
    const fresh = await window.networkFirebaseUtils.fetchAllUsers({ bypassCache: true });
    ...
    nodes.forEach(n => {
      ...
      const u = allUsers.find(x => x.email === n.id);
      if (u) {
        n.avatar = u.avatar || 'static/img/default-avatar.png';
      }
    });

    d3.select('svg').selectAll('image.node-avatar')
      .attr('href', d => d.avatar || 'static/img/default-avatar.png');

    const authUser = window.networkFirebaseUtils.currentUser;
    if (authUser) {
      const me = allUsers.find(u => u.email === authUser.email);
      const avatarSrc = me?.avatar || 'static/img/default-avatar.png';
      ['logged-in-user-avatar', 'mobile-logged-in-user-avatar'].forEach(id => {
        const el = document.getElementById(id);
        if (el) el.src = avatarSrc;
      });
      const popupImg = document.getElementById('popup-image');
      if (popupImg && activeClickedNodeId === authUser.email) {
        popupImg.src = avatarSrc;
      }
    }
  } catch (e) {
    console.warn('[refreshNetworkUserAvatars]', e);
  }
}

async function retryNotificationLoadAfterAuth(trigger) {
  if (!shouldRetryNotificationsAfterAuth || notificationRetryInFlight) return;
  if (!hasInitializedNetworkData) {
    console.log('[notifications] Auth resolved before network data was ready; retry still pending.', { trigger });
    return;
  }
  const authResolved = !!(window.networkFirebaseUtils && window.networkFirebaseUtils.authStateResolved);
  const user = window.networkFirebaseUtils && window.networkFirebaseUtils.currentUser
    ? window.networkFirebaseUtils.currentUser
    : null;
  if (!user) {
    if (!authResolved) {
      console.log('[notifications] Auth retry deferred because auth is still unresolved.', { trigger });
      return;
    }
    console.log('[notifications] Auth retry skipped because auth resolved with no signed-in user.', { trigger });
    shouldRetryNotificationsAfterAuth = false;
    return;
  }

  notificationRetryInFlight = true;
  try {
    console.log('[notifications] Retrying notification load after auth resolved.', {
      trigger,
      email: user.email
    });
    shouldRetryNotificationsAfterAuth = false;
    ...
    notificationRetryInFlight = false;
  }
}

window.addEventListener('kr_network_auth_ready', () => {
  retryNotificationLoadAfterAuth('auth-ready-event');
});

async function bootNetworkPage() {
  if (hasBootstrappedNetworkPage) return;
  hasBootstrappedNetworkPage = true;

  debugLoad('network_app_boot_start', {
    readyState: document.readyState,
    hasHideFn: typeof window.hideNetworkLoadingScreen === 'function'
  });

  try {
    console.log('[DOMContentLoaded] Starting: initNetwork then loadUserNotifications');
    await initNetwork();
    hasInitializedNetworkData = true;
    debugLoad('network_app_boot_after_initNetwork', {
      allUsersCount: allUsers.length,
    ...
    console.log('[DOMContentLoaded] Calling loadUserNotifications (allUsers length:', allUsers.length, ')');
    await loadUserNotifications();
    await retryNotificationLoadAfterAuth('post-initial-load');
    debugLoad('network_app_boot_after_notifications');
    ...
    if (linkFromUrl && allUsers.length > 0) {
      ...
      if (targetUser) {
        debugLoad('network_app_boot_deeplink_target_found', {
          email: targetUser.email
        });
        ...
        if (targetNodeDatum) {
          debugLoad('network_app_boot_deeplink_node_found', {
            nodeId: targetNodeDatum.id
          });
          updateNodeVisuals(targetUser.email);
          setTimeout(() => zoomToNode(targetNodeDatum), 150);
          // Profile modal already shown in initNetwork for faster load
          const logoLink = document.getElementById('fixed-top-logo-link');
          clearFiltersBtn.classList.add('active');
          clearFiltersBtn.style.display = 'block';
          if (logoLink) logoLink.style.pointerEvents = 'none';
        } else {
          debugLoad('network_app_boot_deeplink_node_missing', {
            email: targetUser.email
          });
        ...
        debugLoad('network_app_boot_deeplink_user_missing', {
          linkFromUrl
        });
    ...
    debugLoad('network_app_boot_complete', {
      activeClickedNodeId,
      currentProfileIdInUrl
    });
    // --- END: New code for deep linking ---
  } catch (error) {
    debugLoad('network_app_boot_failed', {
      message: error && error.message ? error.message : String(error)
    });
    console.error('[network_app_boot] failed:', error);
  } finally {
    const loadingOverlay = document.getElementById('network-loading-screen');
    if (
      loadingOverlay &&
      loadingOverlay.dataset.hiding !== '1' &&
      loadingOverlay.style.display !== 'none' &&
      window.getComputedStyle(loadingOverlay).display !== 'none' &&
      typeof window.hideNetworkLoadingScreen === 'function'
    ) {
      debugLoad('network_app_boot_finally_hide_loader');
      window.hideNetworkLoadingScreen();
    }
  }
}
...
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', bootNetworkPage);
} else {
  bootNetworkPage();
}

window.addEventListener('pageshow', () => {
  refreshNetworkUserAvatars();
});
...
try {
  ...
  // Gate hint start: wait for welcome overlay OR profile popup (when ?link= on first visit)
  if (!window.__krWelcomeClosed) {
    window.addEventListener('kr_welcome_closed', startHints, { once: true });
  } else if (window.__krProfileOpenedOnLoad) {
    window.addEventListener('kr_profile_popup_closed', startHints, { once: true });
  } else {
    startHints();
  }
  } catch (e) {
  ...
  const placeholderSearchInput = document.getElementById('network-search-input');
  const searchWrapper = document.querySelector('.network-search-wrapper');

  if (placeholderSearchInput && searchWrapper) {
      ...
      document.addEventListener('visibilitychange', () => {
          ...
          if (document.activeElement !== placeholderSearchInput && !placeholderSearchInput.value) {
              if (document.hidden) {
                  stopCycling();
              } else {
      ...
      placeholderSearchInput.addEventListener('focus', () => {
          stopCycling();
          searchWrapper.classList.add('has-value-or-focus');
      });

      placeholderSearchInput.addEventListener('blur', () => {
          if (!placeholderSearchInput.value) {
              searchWrapper.classList.remove('has-value-or-focus');
              startCycling();
          }
      });

      placeholderSearchInput.addEventListener('input', () => {
          if (placeholderSearchInput.value) {
              searchWrapper.classList.add('has-value-or-focus');
          } else {
      ...
      setTimeout(() => {
          ...
          if (document.activeElement !== placeholderSearchInput && !placeholderSearchInput.value && !document.hidden) {
              changePlaceholder();
              startCycling();
          }
      }, 2500); // Initial delay
...
async function loadUserNotifications() {
    const user = window.networkFirebaseUtils.currentUser;
    console.log('[loadUserNotifications] called, user:', user ? user.email : 'null');
    if (!user) {
        shouldRetryNotificationsAfterAuth = true;
        console.warn('[notifications] Skipping loadUserNotifications because currentUser is not ready yet.', {
            authStateResolved: !!(window.networkFirebaseUtils && window.networkFirebaseUtils.authStateResolved),
            popupHasPlaceholder: !!document.querySelector('#notification-popup-content p'),
            readyState: document.readyState
        });
        return;
    }
    ...
    console.log('[loadUserNotifications] Firestore notifications:', userNotifications.length, 'unread:', unreadNotificationIds.length);

    // Also compute pending connection and collaboration requests so the bell can reflect them
    await computePendingConnectionRequests();
    await computePendingCollaborationRequests();
    console.log('[loadUserNotifications] pendingConnectionRequests:', pendingConnectionRequests.length, '| pendingCollaborationRequests:', pendingCollaborationRequests.length);
...
function updateNotificationBell() {
    ...
    const hasPendingRequests = pendingConnectionRequests.length > 0 || pendingCollaborationRequests.length > 0;
...
function renderNotifications() {
    ...
    if (!popupContent) {
        console.warn('[notifications] renderNotifications aborted because #notification-popup-content was not found.');
        return;
    }
    ...
    if (!listContainer) {
        ...
        console.log('[notifications] Creating notification list container.', {
            removedPlaceholder: !!p,
            notificationCount: userNotifications.length,
            pendingConnectionRequests: pendingConnectionRequests.length,
            pendingCollaborationRequests: pendingCollaborationRequests.length
        });
        if(p) p.remove();
    ...
    // Build Connection Requests and Collaboration Requests sections first
    const connectionRequestsSection = renderConnectionRequestsSection();
    const collaborationRequestsSection = renderCollaborationRequestsSection();
    console.log('[renderNotifications] connectionRequestsSection length:', connectionRequestsSection.length, '| collaborationRequestsSection length:', collaborationRequestsSection.length, '| pendingCollaborationRequests:', pendingCollaborationRequests.length);

    function normalizeNotificationLink(raw) {
        if (!raw) return '';
        try {
            const slugFromUrl = parseProfileSlugFromUrlString(raw);
            if (slugFromUrl) return buildProfileUrl(slugFromUrl);

            let profileVal = null;
            try {
                const u = new URL(raw, window.location.origin);
                profileVal = new URLSearchParams(u.search).get('profile');
            } catch (_) {
            ...
            if (profileVal) {
                ...
                return buildProfileUrl(slugifyTitle(display));
            }
    ...
    const notificationItemsHtml = userNotifications.map(n => {
        ...
        if (n.category === 'Connection Request') {
            ...
            if (n.link) {
                ...
                if (!candidate) {
                    const slug = parseProfileSlugFromUrlString(n.link);
                    if (slug) {
    ...
    const hasAny = (pendingConnectionRequests.length > 0) || (pendingCollaborationRequests.length > 0) || (userNotifications.length > 0);
    if (!hasAny) {
        listContainer.innerHTML = '<p class="no-notifications">You have no notifications.</p>';
        return;
    }

    listContainer.innerHTML = connectionRequestsSection + collaborationRequestsSection + notificationItemsHtml;
...
async function computePendingConnectionRequests() {
    ...
    // Use existing allUsers from initNetwork - no need to fetch again
    const myProfile = await window.networkFirebaseUtils.getUserProfile(user.email);
...
async function computePendingCollaborationRequests() {
    const user = window.networkFirebaseUtils.currentUser;
    if (!user) {
        console.log('[computePendingCollaborationRequests] no user, skipping');
        pendingCollaborationRequests = [];
        return;
    }
    const myProfile = await window.networkFirebaseUtils.getUserProfile(user.email);
    const myCollaborations = Array.isArray(myProfile?.collaborations) ? myProfile.collaborations : [];
    const dismissed = Array.isArray(myProfile?.dismissedCollaborationRequests) ? myProfile.dismissedCollaborationRequests : [];
    console.log('[computePendingCollaborationRequests] myEmail:', user.email, '| myCollaborations:', myCollaborations, '| dismissed:', dismissed, '| allUsers count:', allUsers.length);

    const pending = [];
    let blocksChecked = 0;
    let blocksWithCollab = 0;
    let blocksWhereIAmCollab = 0;
    for (const owner of allUsers) {
        ...
        for (const block of ownerBlocks) {
            blocksChecked++;
            const collab = Array.isArray(block.collaborators) ? block.collaborators : [];
            if (collab.length > 0) blocksWithCollab++;
            if (!collab.includes(user.email)) continue;
            blocksWhereIAmCollab++;
            let gid = block.globalId;
            if (!gid) {
                try {
                    gid = await window.networkFirebaseUtils.ensureBlockHasGlobalId(owner, block, ownerBlocks);
                    console.log('[computePendingCollaborationRequests] Migrated block globalId:', owner.email, 'block:', block.title, 'gid:', gid);
                } catch (err) {
                    console.warn('[computePendingCollaborationRequests] Failed to migrate block:', err);
                    continue;
                }
            }
            if (myCollaborations.includes(gid)) {
                console.log('[computePendingCollaborationRequests] SKIP (already accepted):', owner.email, 'block:', block.title);
                continue;
            }
            if (dismissed.includes(gid)) {
                console.log('[computePendingCollaborationRequests] SKIP (dismissed):', owner.email, 'block:', block.title);
                continue;
            }
            console.log('[computePendingCollaborationRequests] FOUND:', owner.email, 'block:', block.title, 'globalId:', gid);
            pending.push({ owner, block, globalId: gid });
        }
    }
    if (blocksWithCollab > 0 && blocksWhereIAmCollab === 0) {
      const sample = allUsers.flatMap(u => (u.blocks || []).filter(b => (b.collaborators || []).length > 0).map(b => ({ owner: u.email, collaborators: b.collaborators })));
      console.log('[computePendingCollaborationRequests] blocksWithCollab exist but none include my email - sample:', sample, '| myEmail:', user.email);
    }
    console.log('[computePendingCollaborationRequests] done: blocksChecked:', blocksChecked, '| blocksWithCollab:', blocksWithCollab, '| blocksWhereIAmCollab:', blocksWhereIAmCollab, '| pending:', pending.length);
    pendingCollaborationRequests = pending;
}

function renderCollaborationRequestsSection() {
    if (!pendingCollaborationRequests || pendingCollaborationRequests.length === 0) return '';

    const count = pendingCollaborationRequests.length;
    const header = `
        <div class="collaboration-requests-header">
            <h3>Collaboration Requests (${count})</h3>
            <p class="collaboration-requests-note">Someone wants to add you as a collaborator on their post</p>
        </div>
    `;

    const items = pendingCollaborationRequests.map(({ owner, block, globalId }) => {
        const avatar = owner.avatar || 'static/img/default-avatar.png';
        const ownerTitle = owner.title || owner.email.split('@')[0];
        const currentUserEmail = window.networkFirebaseUtils?.currentUser?.email || '';
        const blockTitle = getCollaborationRequestBlockTitle(block, currentUserEmail);
        const viewLink = buildProfileUrl(slugifyTitle(ownerTitle));
        return `
            <div class="notification-item collaboration-request" data-global-id="${globalId}">
                <div class="notification-header">
                    <img src="${getCategoryIcon('Collaboration')}" class="notification-category-icon" alt="Collaboration">
                    <span class="notification-category">Collaboration Request</span>
                </div>
                <div class="notification-content">
                    <img src="${avatar}" alt="${ownerTitle}" class="notification-image">
                    <div class="notification-text">
                        <h4 class="notification-title">${ownerTitle}</h4>
                        <p class="notification-body">wants to add you as collaborator on "${sanitizeText(blockTitle)}"</p>
                        <a href="${viewLink}" class="notification-view-more">View Profile</a>
                    </div>
                </div>
                <div class="connection-request-actions">
                    <button class="collab-accept-btn" data-global-id="${globalId}">Accept</button>
                    <button class="collab-decline-btn" data-global-id="${globalId}">Decline</button>
                </div>
            </div>
        `;
    }).join('');
    ...
    queueMicrotask(() => {
        document.querySelectorAll('.collaboration-request .collab-accept-btn').forEach(btn => {
            btn.addEventListener('click', async (e) => {
                const globalId = e.currentTarget.getAttribute('data-global-id');
                await window.networkFirebaseUtils.addCollaborationForCurrentUser(globalId);
                pendingCollaborationRequests = pendingCollaborationRequests.filter(p => p.globalId !== globalId);
                updateNotificationBell();
                renderNotifications();
                await refreshNetworkGraphAfterSocialAccept();
            });
        });
        document.querySelectorAll('.collaboration-request .collab-decline-btn').forEach(btn => {
            btn.addEventListener('click', async (e) => {
                const globalId = e.currentTarget.getAttribute('data-global-id');
                await window.networkFirebaseUtils.dismissCollaborationRequestForCurrentUser(globalId);
                pendingCollaborationRequests = pendingCollaborationRequests.filter(p => p.globalId !== globalId);
                updateNotificationBell();
                renderNotifications();
            });
...
function renderConnectionRequestsSection() {
    ...
    const items = pendingConnectionRequests.map(u => {
        const avatar = u.avatar || 'static/img/default-avatar.png';
        const title = u.title || u.email.split('@')[0];
        const viewLink = buildProfileUrl(slugifyTitle(title));
        return `
...
function getRenderedNodeRadius(nodeId) {
    if (nodeRadiusCache.has(nodeId)) {
        return nodeRadiusCache.get(nodeId);
    }
    let radius = R_DEFAULT;
    d3.selectAll('.node').filter(d => d.id === nodeId).each(function() {
        const value = parseFloat(d3.select(this).select('.main-circle').attr('r'));
        if (Number.isFinite(value)) radius = value;
    });
    nodeRadiusCache.set(nodeId, radius);
    return radius;
}

function getNodeCollisionRadius(node) {
    return node.type === 'block' ? (BLOCK_SIZE / 2) : getRenderedNodeRadius(node.id);
}

function getVisionNodeDistanceMap() {
    const distanceMap = new Map();
    if (!visionCircleScreenState) return distanceMap;

    const svgNode = d3.select('svg').node();
    const networkContainerG = d3.select('g.network-container').node();
    if (!svgNode || !networkContainerG) return distanceMap;
    const transform = networkContainerG.getScreenCTM();
    if (!transform) return distanceMap;
    ...
    d3.selectAll('.node').each(function(d) {
        ...
        const dx = screenPoint.x - visionCircleScreenState.center.x;
        const dy = screenPoint.y - visionCircleScreenState.center.y;
        distanceMap.set(d.id, Math.hypot(dx, dy));
    });

    return distanceMap;
}

function getVisionContextOpacity(distance, radius, minOpacity = 0.22, maxOpacity = 0.72) {
    if (!Number.isFinite(distance) || !Number.isFinite(radius) || radius <= 0) return maxOpacity;
    const normalized = Math.min(distance / (radius * 2.2), 1);
    const eased = d3.easeCubicOut(1 - normalized);
    return minOpacity + (maxOpacity - minOpacity) * eased;
}

function buildVisionMediaItems(user) {
    const rawItems = user.blocks.flatMap(block => {
        if (block.type === 'carousel' && Array.isArray(block.slides)) {
            return block.slides
                .filter(slide => slide.icon)
                .map(slide => ({ type: 'media', icon: slide.icon, link: slide.link || block.link || null, isEmbed: false }));
        }
        if (block.type === 'embed') {
            const provider = block.provider || (() => {
                try {
                    const url = new URL(String(block.embedUrl || block.link || ''));
            ...
            const icon = provider === 'youtube'
                ? 'static/img/Youtube_logo.png'
                : provider === 'spotify'
                    ? 'static/img/Spotify_logo.png'
                    : 'static/img/default-icon.png';
            return [{ type: 'media', icon, link: null, isEmbed: true }];
        }
        if (block.icon) {
            return [{ type: 'media', icon: block.icon, link: block.link || null, isEmbed: false }];
        }
        return [];
    });

    if (rawItems.length <= VISION_MEDIA_VISIBLE_MAX) {
        return { totalCount: rawItems.length, items: rawItems };
    }

    const visibleItems = rawItems.slice(0, VISION_MEDIA_VISIBLE_ITEMS);
    visibleItems.push({
        type: 'overflow',
        hiddenCount: rawItems.length - VISION_MEDIA_VISIBLE_ITEMS
    });
    return { totalCount: rawItems.length, items: visibleItems };
}

function createVisionMediaItem(parentGroup, item, index, nodeData) {
    const itemGroup = parentGroup.append('g')
        .datum(item)
        .attr('class', `vision-media-item${item.type === 'overflow' ? ' is-overflow' : ''}`)
        .attr('data-media-index', index);
    ...
    if (item.type === 'overflow') {
        itemGroup.append('rect')
            .attr('class', 'media-overflow-tile')
            .attr('x', nodeData.x - VISION_MEDIA_IMAGE_SIZE / 2)
            .attr('y', nodeData.y - VISION_MEDIA_IMAGE_SIZE / 2)
            .attr('width', VISION_MEDIA_IMAGE_SIZE)
            .attr('height', VISION_MEDIA_IMAGE_SIZE)
            .attr('rx', 8)
            .attr('ry', 8);

        itemGroup.append('text')
            .attr('class', 'media-overflow-text')
            .attr('x', nodeData.x)
            .attr('y', nodeData.y)
            .text(`+${item.hiddenCount}`);
        return;
    }
    ...
    if (item.isEmbed) {
        itemGroup.append('circle')
            ...
            .attr('cx', nodeData.x)
            .attr('cy', nodeData.y)
            .attr('r', VISION_MEDIA_IMAGE_SIZE / 2)
...
function buildVisionMediaLayoutPositions(nodeData, itemCount, centerAngle, nodeRadius) {
    const rows = itemCount <= 3 ? [itemCount] : itemCount === 4 ? [2, 2] : [3, 2];
    const positions = [];
    const baseRadius = nodeRadius + VISION_MEDIA_NODE_GAP + VISION_MEDIA_IMAGE_SIZE / 2;

    rows.forEach((rowCount, rowIndex) => {
        const rowRadius = baseRadius + rowIndex * VISION_MEDIA_ROW_STEP;
        const rowSpread = rowCount === 1 ? 0 : Math.min(Math.PI / 2.6, 0.35 + rowCount * 0.18);
        for (let i = 0; i < rowCount; i++) {
            const offset = rowCount === 1 ? 0 : (-rowSpread / 2) + (rowSpread * i / (rowCount - 1));
            const angle = centerAngle + offset;
            positions.push({
                x: nodeData.x + rowRadius * Math.cos(angle),
                y: nodeData.y + rowRadius * Math.sin(angle)
            });
        }
    });
    ...
    const radius = positions.reduce((maxRadius, pos) => {
        const distance = Math.hypot(pos.x - center.x, pos.y - center.y);
        return Math.max(maxRadius, distance + VISION_MEDIA_IMAGE_SIZE / 2 + 8 + VISION_MEDIA_HALO_PADDING);
    }, VISION_MEDIA_IMAGE_SIZE * 0.8 + VISION_MEDIA_HALO_PADDING);

    return { positions, center, radius };
}

function scoreVisionMediaLayout(nodeData, state, candidateAngle, placedClusters, nodeCollisionData) {
    const layout = buildVisionMediaLayoutPositions(nodeData, state.items.length, candidateAngle, getRenderedNodeRadius(nodeData.id));
    let score = 0;

    const otherClusters = [
        ...placedClusters,
        ...Array.from(visionMediaState.entries())
            .filter(([otherNodeId, otherState]) => otherNodeId !== nodeData.id && otherState.layout)
            .map(([otherNodeId, otherState]) => ({ nodeId: otherNodeId, ...otherState.layout }))
    ];

    layout.positions.forEach(pos => {
        nodeCollisionData.forEach(otherNode => {
            if (otherNode.id === nodeData.id) return;
            const minDistance = otherNode.radius + VISION_MEDIA_IMAGE_SIZE * 0.7 + 10;
            const distance = Math.hypot(pos.x - otherNode.x, pos.y - otherNode.y);
            if (distance < minDistance * 3) {
                const overlap = (minDistance * 3) - distance;
                score += overlap * overlap;
            }
        });

        otherClusters.forEach(cluster => {
            const distance = Math.hypot(pos.x - cluster.center.x, pos.y - cluster.center.y);
            const minDistance = cluster.radius + VISION_MEDIA_IMAGE_SIZE * 0.8;
            if (distance < minDistance) {
                const overlap = minDistance - distance;
                score += overlap * overlap * 3;
            }
        });
    });

    if (visionCircleSimState) {
        const outwardAngle = Math.atan2(nodeData.y - visionCircleSimState.center.y, nodeData.x - visionCircleSimState.center.x);
        const outwardBias = Math.abs(shortestAngleDelta(candidateAngle, outwardAngle));
        score += outwardBias * 16;
    }

    return { layout, score };
}

function positionVisionMediaGroup(nodeData, state) {
    if (!state.layout) return;

    state.group.select('.vision-media-halo')
        .attr('cx', state.layout.center.x)
        .attr('cy', state.layout.center.y)
        .attr('r', state.layout.radius);

    state.group.selectAll('.vision-media-item').each(function(item, index) {
        const itemGroup = d3.select(this);
        const position = state.layout.positions[index];
        if (!position) return;

        itemGroup.select('.media-link')
            .attr('x1', nodeData.x)
            .attr('y1', nodeData.y)
            .attr('x2', position.x)
            .attr('y2', position.y);
        ...
        if (item.type === 'overflow') {
            itemGroup.select('.media-overflow-tile')
                .attr('x', position.x - VISION_MEDIA_IMAGE_SIZE / 2)
                .attr('y', position.y - VISION_MEDIA_IMAGE_SIZE / 2);
            itemGroup.select('.media-overflow-text')
                .attr('x', position.x)
                .attr('y', position.y + 3);
            return;
        }

        itemGroup.select('.vision-media-image')
            .attr('x', position.x - VISION_MEDIA_IMAGE_SIZE / 2)
            .attr('y', position.y - VISION_MEDIA_IMAGE_SIZE / 2);
        itemGroup.select('.media-image-border')
            .attr('cx', position.x)
            .attr('cy', position.y);
    });
}

function updateVisionMediaLayouts(forceRecompute = false) {
    if (!isVisionModeActive || !visionMediaGroup || visionMediaState.size === 0) return;
    visionLayoutTick += 1;

    const nodeCollisionData = nodes.map(node => ({
        id: node.id,
        x: node.x,
        y: node.y,
    ...
    const activeStates = Array.from(visionMediaState.entries())
        .map(([nodeId, state]) => ({
            nodeId,
            state,
    ...
    activeStates.sort((a, b) => (a.state.visionDistance ?? Infinity) - (b.state.visionDistance ?? Infinity));

    const placedClusters = [];

    activeStates.forEach(({ nodeData, state, nodeId }) => {
        let bestAngle = state.lastAngle ?? 0;
        let nextLayout = state.layout;
        const ownerMovedDistance = state.lastOwnerPosition
            ? Math.hypot(nodeData.x - state.lastOwnerPosition.x, nodeData.y - state.lastOwnerPosition.y)
            : Number.POSITIVE_INFINITY;
        const shouldRecomputeAngle = forceRecompute
            || !state.layout
            || state.lastAngle == null
            || ownerMovedDistance >= VISION_LAYOUT_MOVE_THRESHOLD
            || !state.lastLayoutTick
            || (visionLayoutTick - state.lastLayoutTick) >= VISION_LAYOUT_RECALC_INTERVAL;

        if (shouldRecomputeAngle) {
            let bestLayout = null;
            let bestScore = Number.POSITIVE_INFINITY;

            for (let sample = 0; sample < VISION_MEDIA_ANGLE_SAMPLES; sample++) {
                const candidateAngle = (sample / VISION_MEDIA_ANGLE_SAMPLES) * Math.PI * 2;
                const candidate = scoreVisionMediaLayout(nodeData, state, candidateAngle, placedClusters, nodeCollisionData);
            ...
            const smoothedAngle = state.lastAngle == null
                ? bestAngle
                : normalizeAngle(state.lastAngle + shortestAngleDelta(state.lastAngle, bestAngle) * VISION_MEDIA_HYSTERESIS);

            state.lastAngle = smoothedAngle;
            nextLayout = bestLayout && Math.abs(shortestAngleDelta(smoothedAngle, bestAngle)) < 0.01
                ? bestLayout
                : buildVisionMediaLayoutPositions(nodeData, state.items.length, smoothedAngle, getRenderedNodeRadius(nodeData.id));
            state.lastLayoutTick = visionLayoutTick;
        } else {
            nextLayout = buildVisionMediaLayoutPositions(nodeData, state.items.length, state.lastAngle, getRenderedNodeRadius(nodeData.id));
        }

        state.layout = nextLayout;
        state.lastOwnerPosition = { x: nodeData.x, y: nodeData.y };
        placedClusters.push({ nodeId, ...state.layout });
        positionVisionMediaGroup(nodeData, state);
    });
}

function applyVisionContextFade() {
    if (!isVisionModeActive || visionMediaState.size === 0 || !visionCircleScreenState) {
        d3.selectAll('.node-title').style('opacity', null);
        d3.selectAll('.link').style('opacity', null);
        d3.selectAll('.collab-link').style('opacity', null).style('stroke-opacity', null);
        d3.selectAll('.practice-link').style('opacity', null);
        return;
    }

    const visibleMediaNodeIds = new Set(visionMediaState.keys());
    const nodeDistanceMap = getVisionNodeDistanceMap();

    d3.selectAll('.node-title').style('opacity', function(d) {
        if (visibleMediaNodeIds.has(d.id)) return 1;
        const distance = nodeDistanceMap.get(d.id);
        return getVisionContextOpacity(distance, visionCircleScreenState.radius, 0.24, 0.7);
    });

    d3.selectAll('.link').style('opacity', function(l) {
        const distance = Math.min(
            nodeDistanceMap.get(l.source.id) ?? Number.POSITIVE_INFINITY,
            nodeDistanceMap.get(l.target.id) ?? Number.POSITIVE_INFINITY
        );
        return getVisionContextOpacity(distance, visionCircleScreenState.radius, 0.16, 0.55);
    });

    d3.selectAll('.collab-link').style('stroke-opacity', function(l) {
        const distance = Math.min(
            nodeDistanceMap.get(l.source.id) ?? Number.POSITIVE_INFINITY,
            nodeDistanceMap.get(l.target.id) ?? Number.POSITIVE_INFINITY
        );
        return getVisionContextOpacity(distance, visionCircleScreenState.radius, 0.14, 0.42);
    });

    d3.selectAll('.practice-link').style('opacity', function(l) {
        const distance = Math.min(
            nodeDistanceMap.get(l.source.id) ?? Number.POSITIVE_INFINITY,
            nodeDistanceMap.get(l.target.id) ?? Number.POSITIVE_INFINITY
        );
        return getVisionContextOpacity(distance, visionCircleScreenState.radius, 0.12, 0.3);
    });
}

function resetVisionContextFade() {
    d3.selectAll('.node-title').style('opacity', null);
    d3.selectAll('.link').style('opacity', null);
    d3.selectAll('.collab-link').style('opacity', null).style('stroke-opacity', null);
    d3.selectAll('.practice-link').style('opacity', null);
...
function toggleVisionMode() {
    ...
    if (isVisionModeActive) {
        ...
        nodesInVision.clear();
        visionLayoutTick = 0;
        visionCircleScreenState = null;
        visionCircleSimState = null;
        // Reset node sizes
        updateNodeVisuals(activeClickedNodeId, new Map());
        resetVisionContextFade();
...
function updateVisionMode() {
    ...
    const [simCenterX, simCenterY] = zoomTransform.invert([circleCenter.x, circleCenter.y]);
    visionCircleSimState = {
        center: { x: simCenterX, y: simCenterY },
        radius: circleRadius / zoomTransform.k
    };
    ...
    d3.selectAll('.node').each(function(d) {
        ...
        if (distance <= circleRadius) {
            ...
            if (visionMediaState.has(d.id)) {
                visionMediaState.get(d.id).visionDistance = distance;
            }
        }
    });
...
function magnifyingForce(alpha) {
    ...
    visionMediaState.forEach((state, ownerNodeId) => {
        if (!state.layout) return;

        for (const node of nodes) {
            if (node.id === ownerNodeId) continue;

            const dx = node.x - state.layout.center.x;
            const dy = node.y - state.layout.center.y;
            const distance = Math.hypot(dx, dy) || 0.001;
            const minDistance = state.layout.radius + getNodeCollisionRadius(node) + 10;
            if (distance < minDistance) {
                const push = (minDistance - distance) * 0.015 * alpha;
...
function showNodeMedia(nodeData) {
    if (visionMediaState.has(nodeData.id)) return;

    const user = allUsers.find(u => u.email === nodeData.id);
    if (!user || !user.blocks || user.blocks.length === 0) return;

    const mediaConfig = buildVisionMediaItems(user);
    if (mediaConfig.items.length === 0) return;
    ...
    parentGroup.append('circle')
        .attr('class', 'vision-media-halo')
        .attr('cx', nodeData.x)
        .attr('cy', nodeData.y)
        .attr('r', 0);

    mediaConfig.items.forEach((item, index) => {
        createVisionMediaItem(parentGroup, item, index, nodeData);
    });

    visionMediaState.set(nodeData.id, {
        group: parentGroup,
        items: mediaConfig.items,
        totalCount: mediaConfig.totalCount,
        lastAngle: null,
        layout: null,
        visionDistance: null,
        lastLayoutTick: 0,
...
function hideNodeMedia(nodeId) {
    const state = visionMediaState.get(nodeId);
    const group = state ? state.group : visionMediaGroup.selectAll(`.vision-media-group[data-node-id="${nodeId}"]`);
    visionMediaState.delete(nodeId);
...
function calculatePanningMagnification(transform) {
    ...
    const centerY = fixedHeaderHeight + (svgNode.clientHeight - fixedHeaderHeight) / 2.7;
