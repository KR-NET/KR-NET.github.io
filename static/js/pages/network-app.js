...
async function initNetwork() {
  ...
  // --- Early profile display when ?link=slug: show popup immediately, don't wait for graph ---
  ...
  // Collect collaborative blocks: owner + at least 1 accepted collaborator, all in visible network
  collabBlocks = [];
  collabLinks = [];
    ...
    (user.blocks || []).forEach(block => {
      ...
      if (block.type === 'carousel' && Array.isArray(block.slides)) {
        block.slides.forEach((slide, slideIdx) => {
          ...
          const accepted = getAcceptedCollaboratorsForEmails(slideCollaborators, block.globalId);
      ...
      const accepted = getAcceptedCollaboratorsForEmails(block.collaborators, block.globalId);
      if (accepted.length === 0) return;
      const allParties = [user.email, ...accepted.map(u => u.email)];
      const allVisible = allParties.every(email => visibleEmails.has(email));
      if (!allVisible) return;
  ...
  // Add block nodes and collab links to simulation (blocks first so they render behind user nodes)
  nodes = [...collabBlocks, ...nodes];
  links = [...links, ...collabLinks];
  ...
  simulation = d3.forceSimulation(nodes)
    .force("link", d3.forceLink(links)
      .id(d => d.id)
      .distance(link => {
        if (link.isCollab) return 35; // Shorter distance for collaborators
        if (link.hasSharedCollab) return 35; // Pull collaborating users closer together
      ...
      .strength(link => {
        if (link.isCollab) return 0.85; // Stronger gravity for collaborators
        if (link.hasSharedCollab) return 0.7; // Shared collaboration links cluster more tightly
        const sourceDegree = nodeDegrees[link.source.id] || 1;
        const targetDegree = nodeDegrees[link.target.id] || 1;
  ...
  const collabPath = collabLinksLayer
    .selectAll("path")
    .data(collabLinks)
    .join("path")
  ...
  // Block nodes: rect + image (rectangular)
  const blockNode = node.filter(d => d.type === 'block');
  const half = BLOCK_SIZE / 2;
  blockNode.append("rect")
    .attr("x", -half)
    .attr("y", -half)
  ...
  // User nodes: circle + avatar + title
  const userNode = node.filter(d => d.type !== 'block');
  userNode.append("defs")
    .append("clipPath")
  ...
  // Orthogonal path helper: leave the block from the edge facing the target,
  // so connectors always emerge perpendicularly from the midpoint of that side.
  ...
  simulation.on("tick", () => {
    ...
    collabPath.attr("d", orthogonalPath);
  ...
  // Add click handler for nodes (user + block)
...
function getAcceptedCollaboratorsForEmails(emails, globalId) {
...
function renderCollaboratorsHtmlForEmails(emails, globalId) {
  ...
  const chips = accepted.map(u => {
    ...
    const profileLink = `https://kr-net.work?link=${slug}`;
    return `
...
function renderCollaborationRequestsSection() {
    ...
    const items = pendingCollaborationRequests.map(({ owner, block, globalId }) => {
        ...
        const viewLink = `https://kr-net.work?link=${slugifyTitle(ownerTitle)}`;
        return `
