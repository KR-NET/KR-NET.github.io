...
async function initNetwork() {
  ...
  visibleUsers.forEach(user => {
    (user.blocks || []).forEach(block => {
      if (!block.globalId || !Array.isArray(block.collaborators) || block.collaborators.length === 0) return;
      const accepted = block.collaborators
      ...
      allParties.forEach((targetEmail, idx) => {
        collabLinks.push({
          source: block.globalId,
          target: targetEmail,
          isCollab: true,
          exitIndex: idx % 4 // 0=top, 1=right, 2=bottom, 3=left
        });
      });
    });
  });
  ...
  // Orthogonal path helper: straight H/V segments, rounded corners via stroke-linejoin
  function orthogonalPath(d) {
    ...
    const ty = tgt.y;
    const half = BLOCK_SIZE / 2;
    const ei = d.exitIndex || 0;
    let ex, ey;
    if (ei === 0) { ex = bx; ey = by - half; }
    else if (ei === 1) { ex = bx + half; ey = by; }
    else if (ei === 2) { ex = bx; ey = by + half; }
    else { ex = bx - half; ey = by; }
    if (ei === 0 || ei === 2) return `M ${ex} ${ey} H ${tx} V ${ty}`;
...
function renderCollaboratorsHtml(block) {
  const accepted = getAcceptedCollaborators(block);
  if (accepted.length === 0) return '';
  ...
  if (user.blocks && user.blocks.length > 0) {
    user.blocks.forEach((block, blockIdx) => {
      if (block.type === 'default' || !block.type) {
        ...
        const collabHtmlCarousel = renderCollaboratorsHtml(block);
        blockContainer.innerHTML += `
            ...
            <div class="notification-item collaboration-request" data-global-id="${globalId}">
                ...
                <div class="notification-content">
                    ...
                    <div class="notification-text">
                        ...
                        <p class="notification-body">wants to add you as collaborator on "${blockTitle}"</p>
