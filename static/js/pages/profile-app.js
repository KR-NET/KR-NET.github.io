...
import { invalidateUsersSessionCache } from "../firebase/users-cache.js";
import { validateDisplayTitle, slugifyTitle, TITLE_MAX_LENGTH } from "../utils/slugify.js";
import { buildProfileUrl } from "../utils/profile-url.js";
import {
  truncateBlockTitle,
  truncateBlockDesc,
  applyBlockTitleValue,
  applyBlockDescValue,
  initBlockTitleField,
  initBlockDescField,
  initBlockSlideTextFields,
  isBlockEmptyForGlobalPublish,
} from "../utils/block-limits.js";
...
const onboardHeader = onboardTitle?.closest('.onboard-header') || null;
...
const onboardSwitchUser = document.getElementById('onboard-switch-user');
...
let displayNameAvailability = { status: 'idle', message: '' }; // idle | checking | ok | error
...
function validateStep(data) {
  ...
  switch (onboardStep) {
    case 0: {
      ...
      const validation = validateDisplayTitle(v);
      if (!validation.ok) return false;
      if (displayNameAvailability.status === 'checking') return false;
      if (displayNameAvailability.status === 'error') return false;
      const existingSlug = doc.titleSlug || (doc.title ? slugifyTitle(doc.title) : '');
      if (validation.slug === existingSlug) return true;
      return displayNameAvailability.status === 'ok';
    }
    case 1: {
...
function renderWelcomeOnboardTitle() {
  const email = (window.firebaseUtils?.currentUser?.email || '').trim();
  if (!onboardTitle) return;
  if (!email) {
    onboardTitle.textContent = 'Welcome to KR, let\u2019s create your profile!';
    return;
  }
  onboardTitle.innerHTML = `<span class="onboard-title-email">${escapeHtml(email)}</span><br>Welcome to KR, let\u2019s create your profile!`;
}

function renderSwitchUserButton() {
  if (!onboardSwitchUser) return;
  const show = onboardStep === 0;
  onboardSwitchUser.style.display = show ? 'inline-block' : 'none';
  onboardHeader?.classList.toggle('has-switch-user', show);
}
...
function renderStep() {
  ...
  if (onboardStep === 0) {
    onboardBody.innerHTML = `
      ...
      <input id="ob-title" class="input-feild onboard-input" type="text" placeholder="Profile Title" maxlength="${TITLE_MAX_LENGTH}" value="${doc.title || ''}">
      <div class="onboard-hint" id="ob-title-hint"></div>
    `;
    const titleInput = document.getElementById('ob-title');
    displayNameAvailability = { status: 'idle', message: '' };
    titleInput.addEventListener('input', () => {
      debouncedAutoSave();
      localStorage.setItem(OB_SEEN_KEY, String(onboardStep));
      scheduleDisplayNameCheck(titleInput.value);
    });
    ...
    if (initialTitle) scheduleDisplayNameCheck(initialTitle);
    else updateValidation();
  } else if (onboardStep === 1) {
    const bioVal = (doc.bio || '').slice(0, 200);
    const count = bioVal.length;
    ...
    onboardBody.innerHTML = `
      ...
      <textarea id="ob-bio" class="input-feild onboard-input" rows="5" placeholder="Add your bio..." maxlength="200">${bioVal}</textarea>
      <div><span class="onboard-hint" id="ob-bio-hint"></span><span class="onboard-counter" id="ob-bio-count">${count}/200</span></div>
    `;
    ...
    bioInput.addEventListener('input', () => {
      ...
      bioCount.textContent = `${v.length}/200`;
      if (v.length > 200) bioInput.value = v.slice(0, 200);
...
function updateValidation() {
  ...
  if (onboardStep === 0) {
    ...
    const hint = document.getElementById('ob-title-hint');
    if (!hint) return;
    const validation = validateDisplayTitle(v);
    if (!v) {
      hint.textContent = 'Add your display name to continue.';
    } else if (!validation.ok) {
      hint.textContent = validation.error;
    } else if (displayNameAvailability.status === 'checking') {
      hint.textContent = displayNameAvailability.message || 'Checking availability…';
    } else if (displayNameAvailability.status === 'error') {
      hint.textContent = displayNameAvailability.message;
    } else {
      hint.textContent = '';
    }
  } else if (onboardStep === 1) {
...
onboardSwitchUser?.addEventListener('click', () => {
    ...
    img.onload = () => {
      ...
      modal.querySelector("#crop-confirm").onclick = async () => {
        ...
        try {
          ...
          const email = window.firebaseUtils.currentUser.email;
          const authUid = window.firebaseUtils.requireAuthUid();
          const avatarPath = window.firebaseUtils.avatarStoragePath(authUid, `${Date.now()}.jpg`);
          const avatarUrl = await window.firebaseUtils.uploadImage(compressedFile, avatarPath);
          ...
          invalidateUsersSessionCache();

          cropper.destroy();
          modal.remove();
        } catch (error) {
          console.error('Error saving avatar:', error);
          const msg = error && error.code === 'storage/unauthorized'
            ? 'Upload blocked by Storage rules. Paste the updated rules from storage.rules into Firebase Console → Storage → Rules, then try again.'
...
function updateProfileTitleHint() {
  const hint = document.getElementById('profile-title-hint');
  const input = document.getElementById('profile-title');
...
function scheduleDisplayNameCheck(title) {
  clearTimeout(displayNameCheckTimer);
  const trimmed = String(title || '').trim();
  const validation = validateDisplayTitle(trimmed);
  if (!trimmed || !validation.ok) {
    displayNameAvailability = validation.ok
      ? { status: 'idle', message: '' }
      : { status: 'error', message: validation.error };
    updateValidation();
    updateProfileTitleHint();
    return;
  }
  displayNameAvailability = { status: 'checking', message: 'Checking availability…' };
  updateValidation();
  updateProfileTitleHint();
  displayNameCheckTimer = setTimeout(() => runDisplayNameCheck(trimmed), 450);
}

async function runDisplayNameCheck(title) {
  const email = window.firebaseUtils?.currentUser?.email;
  if (!email || !window.firebaseUtils?.checkDisplayNameAvailable) {
    displayNameAvailability = { status: 'idle', message: '' };
    updateValidation();
    updateProfileTitleHint();
    return;
  ...
  try {
    const result = await window.firebaseUtils.checkDisplayNameAvailable(title, email);
    displayNameAvailability = result.available
      ? { status: 'ok', message: '' }
      : { status: 'error', message: result.reason || 'That name is already on the network.' };
  } catch (_) {
    displayNameAvailability = { status: 'error', message: 'Could not check name. Try again.' };
  }
  updateValidation();
  updateProfileTitleHint();
}
...
async function autoSaveProfile() {
  ...
  try {
    ...
    const titleVal = document.getElementById("profile-title").value;
    const titleValidation = validateDisplayTitle(titleVal);

    const profileData = {
      bio: (document.getElementById("bio").value || "").slice(0, 200),
    ...
    if (titleVal.trim()) {
      if (!titleValidation.ok) {
        saveStatusEl.textContent = titleValidation.error;
        saveStatusEl.classList.add('visible');
        saveStatusEl.classList.remove('saved');
        setTimeout(() => saveStatusEl.classList.remove('visible'), 2500);
        return;
      }
      if (displayNameAvailability.status === 'error') {
        saveStatusEl.textContent = displayNameAvailability.message;
        saveStatusEl.classList.add('visible');
        saveStatusEl.classList.remove('saved');
        setTimeout(() => saveStatusEl.classList.remove('visible'), 2500);
        return;
      }
      profileData.title = titleVal;
    }
    ...
    saveStatusEl.textContent = e.message || 'Error saving';
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (profileTitleInput) {
    profileTitleInput.addEventListener('input', () => {
      scheduleDisplayNameCheck(profileTitleInput.value);
    });
  ...
  // Bio character limit (200) - enforce and show counter
  const bioField = document.getElementById('bio');
  ...
  if (bioField && bioCountEl) {
    const updateBioCounter = () => {
      ...
      bioCountEl.textContent = `${len}/200`;
      if (len > 200) bioField.value = bioField.value.slice(0, 200);
    };
    bioField.addEventListener('input', updateBioCounter);
    updateBioCounter(); // Set initial count
  }
});
...
const blockCollaboratorsInput = document.getElementById('block-collaborators');
...
const collabDropdownResults = document.getElementById('collab-dropdown-results');
const collabDoneBtn = document.getElementById('collab-done-btn');
const collabSelectedChips = document.getElementById('collab-selected-chips');
const collabSelector = document.getElementById('collab-selector');
const blockTitleCount = document.getElementById('block-title-count');
const saveBlockBtn = document.getElementById('save-block');
const SAVE_BLOCK_DISABLED_TITLE = 'Enter a valid URL and title to save';
const blockDescCount = document.getElementById('block-description-count');

let blocks = [];
let selectedCollaborators = []; // { email, title, avatar }
let collabSearchDebounceTimer = null;
let collabHighlightedIndex = -1;
let profileAllUsersCache = []; // Cached for collaborator avatars in live preview
...
function normalizeCollaboratorEmails(emails) {
...
function buildCollaboratorEntries(emails) {
  return normalizeCollaboratorEmails(emails).map(email => {
    const user = profileAllUsersCache.find(u => u.email === email);
    return user
      ? { email: user.email, title: user.title, avatar: user.avatar }
      : { email, title: email.split('@')[0], avatar: DEFAULT_ICON };
  });
}

function getCarouselCollaboratorEmailsFromSlides(slides) {
  if (!Array.isArray(slides)) return [];
  return normalizeCollaboratorEmails(
    slides.flatMap(slide => Array.isArray(slide?.collaborators) ? slide.collaborators : [])
  );
}

function getBlockCollaboratorEmails(block) {
  if (!block) return [];
  if (block.type === 'carousel' && Array.isArray(block.slides)) {
    return getCarouselCollaboratorEmailsFromSlides(block.slides);
  }
  return normalizeCollaboratorEmails(block.collaborators);
}

function getPendingCollaboratorEmails(emails, globalId, allUsers = []) {
  const normalizedEmails = normalizeCollaboratorEmails(emails);
  if (!globalId || !Array.isArray(allUsers) || allUsers.length === 0) {
    return normalizedEmails;
  }
  return normalizedEmails.filter(email => {
    const user = allUsers.find(entry => entry.email === email);
    return !user || !(Array.isArray(user.collaborations) ? user.collaborations : []).includes(globalId);
  });
}

// Shared helper: render collaborators preview HTML (avatar from allUsers when available)
function renderCollaboratorsPreviewHtml(emails, options = {}) {
  const { pendingEmails = [], allUsers = [], extraClass = '' } = options;
  if (!emails || !Array.isArray(emails) || emails.length === 0) return '';
  const chips = emails.map(email => {
    const e = String(email || '').trim();
    if (!e) return '';
    const user = allUsers.find(u => u.email === e);
    const avatar = user?.avatar || DEFAULT_ICON;
    const displayName = user?.title || e.split('@')[0] || '?';
    const isPending = pendingEmails.includes(e);
    const escaped = escapeCollabHtml(displayName);
    return `
      <div class="collaborator-chip ${isPending ? 'collaborator-chip-pending' : ''}">
        <img src="${avatar}" alt="${escaped}" class="collaborator-chip-avatar">
        <span class="collaborator-chip-info">
          <span class="collaborator-chip-name">${escaped}</span>
          ${isPending ? '<span class="collab-pending-indicator">Pending</span>' : ''}
        </span>
      </div>
    `;
  }).filter(Boolean).join('');
  if (!chips) return '';
  return `
    <div class="block-collaborators ${extraClass}">
...
// --- Collaborator selector (Instagram-like search & select) ---
function syncCollabToHiddenInput() {
  if (blockCollaboratorsInput) {
    blockCollaboratorsInput.value = selectedCollaborators.map(c => c.email).join(', ');
    blockCollaboratorsInput.dispatchEvent(new Event('input', { bubbles: true }));
  }
}

function renderCollabSelectedChips() {
  if (!collabSelectedChips) return;
  if (selectedCollaborators.length === 0) {
    collabSelectedChips.innerHTML = '';
    collabSelectedChips.style.display = 'none';
    return;
  }
  collabSelectedChips.style.display = 'flex';
  collabSelectedChips.innerHTML = selectedCollaborators.map(c => {
    const escaped = (c.title || c.email).replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;');
    return `<span class="collab-selector-chip" data-email="${(c.email || '').replace(/"/g, '&quot;')}">
      <img src="${c.avatar || DEFAULT_ICON}" alt="" class="collab-selector-chip-avatar">
      <span class="collab-selector-chip-name">${escaped}</span>
      <button type="button" class="collab-selector-chip-remove" aria-label="Remove">×</button>
    </span>`;
  }).join('');
  collabSelectedChips.querySelectorAll('.collab-selector-chip-remove').forEach(btn => {
    btn.addEventListener('click', (e) => {
      e.stopPropagation();
      const chip = btn.closest('.collab-selector-chip');
      const email = chip?.getAttribute('data-email');
      if (email) {
        selectedCollaborators = selectedCollaborators.filter(c => c.email !== email);
        renderCollabSelectedChips();
        syncCollabToHiddenInput();
        if (typeof renderBlockLivePreview === 'function') renderBlockLivePreview();
      }
    });
  });
}

function filterCollabSearchResults(term) {
  const t = (term || '').toLowerCase().trim();
  const currentEmail = window.firebaseUtils?.currentUser?.email;
  const excludeSelf = profileAllUsersCache.filter(u => u.email !== currentEmail);
  if (!t) return excludeSelf.slice(0, 15);
  return excludeSelf
    .filter(u => (u.title || '').toLowerCase().includes(t))
    .slice(0, 15);
}

function renderCollabDropdown(searchTerm) {
  if (!collabDropdownResults) return;
  const results = filterCollabSearchResults(searchTerm);
  const selectedEmails = new Set(selectedCollaborators.map(c => c.email));
  if (results.length === 0) {
    collabDropdownResults.innerHTML = '<div class="collab-dropdown-empty">No matching users</div>';
    collabHighlightedIndex = -1;
    return;
  }
  collabDropdownResults.innerHTML = results.map((u, i) => {
    const isSelected = selectedEmails.has(u.email);
    const title = u.title || u.email.split('@')[0] || '?';
    const escaped = title.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;');
    return `<div class="collab-dropdown-row ${isSelected ? 'selected' : ''}" data-index="${i}" data-email="${(u.email || '').replace(/"/g, '&quot;')}">
      <img src="${u.avatar || DEFAULT_ICON}" alt="" class="collab-dropdown-avatar">
      <span class="collab-dropdown-title">${escaped}</span>
      <span class="collab-dropdown-check">${isSelected ? '✓' : ''}</span>
    </div>`;
  }).join('');
  collabHighlightedIndex = -1;
  collabDropdownResults.querySelectorAll('.collab-dropdown-row').forEach(row => {
    row.addEventListener('click', () => {
      const email = row.getAttribute('data-email');
      const u = results.find(r => r.email === email);
      if (!u) return;
      const idx = selectedCollaborators.findIndex(c => c.email === email);
      if (idx >= 0) {
        selectedCollaborators.splice(idx, 1);
      } else {
        selectedCollaborators.push({ email: u.email, title: u.title, avatar: u.avatar });
      }
      renderCollabSelectedChips();
      syncCollabToHiddenInput();
      if (collabSearchInput) collabSearchInput.value = '';
...
function showCollabDropdown() {
  if (collabDropdown) collabDropdown.classList.add('open');
  const term = collabSearchInput?.value || '';
  renderCollabDropdown(term);
  const rows = collabDropdownResults?.querySelectorAll('.collab-dropdown-row');
  if (rows?.length) {
    collabHighlightedIndex = 0;
    rows[0]?.classList.add('highlighted');
  }
}

function hideCollabDropdown() {
  if (collabDropdown) collabDropdown.classList.remove('open');
  collabHighlightedIndex = -1;
}

function initCollabSelector() {
  if (!collabSearchInput || !collabDropdown) return;

  collabSearchInput.addEventListener('focus', () => {
    if (profileAllUsersCache.length === 0 && window.firebaseUtils?.getAllUsersArray) {
      window.firebaseUtils.getAllUsersArray().then(users => {
        profileAllUsersCache = users;
        showCollabDropdown();
      }).catch(() => {});
    } else {
      showCollabDropdown();
  ...
  collabSearchInput.addEventListener('input', () => {
    clearTimeout(collabSearchDebounceTimer);
    collabSearchDebounceTimer = setTimeout(() => {
      renderCollabDropdown(collabSearchInput.value);
    }, 300);
  });

  collabSearchInput.addEventListener('keydown', (e) => {
    if (!collabDropdown?.classList.contains('open')) return;
    const rows = collabDropdownResults?.querySelectorAll('.collab-dropdown-row');
    if (!rows?.length) {
      if (e.key === 'Escape') hideCollabDropdown();
      return;
    }
    if (e.key === 'ArrowDown') {
      e.preventDefault();
      collabHighlightedIndex = Math.min(collabHighlightedIndex + 1, rows.length - 1);
      rows[collabHighlightedIndex]?.scrollIntoView({ block: 'nearest' });
      rows.forEach((r, i) => r.classList.toggle('highlighted', i === collabHighlightedIndex));
    } else if (e.key === 'ArrowUp') {
      e.preventDefault();
      collabHighlightedIndex = Math.max(collabHighlightedIndex - 1, 0);
      rows[collabHighlightedIndex]?.scrollIntoView({ block: 'nearest' });
      rows.forEach((r, i) => r.classList.toggle('highlighted', i === collabHighlightedIndex));
    } else if (e.key === 'Enter' && collabHighlightedIndex >= 0 && rows[collabHighlightedIndex]) {
      e.preventDefault();
      rows[collabHighlightedIndex].click();
    } else if (e.key === 'Escape') {
      e.preventDefault();
      hideCollabDropdown();
      collabSearchInput.blur();
    }
  });

  if (collabDoneBtn) {
    collabDoneBtn.addEventListener('click', () => {
      hideCollabDropdown();
  ...
  document.addEventListener('click', (e) => {
    if (collabSelector && !collabSelector.contains(e.target)) hideCollabDropdown();
  });
}

function initCarouselSlideCollabSelector(slideField, slide) {
  const collabSelectorEl = slideField.querySelector('.carousel-slide-collab-selector');
  if (!collabSelectorEl) return;

  const selectedChipsEl = collabSelectorEl.querySelector('.collab-selected-chips');
  const searchInputEl = collabSelectorEl.querySelector('.collab-search-input');
  const dropdownEl = collabSelectorEl.querySelector('.collab-dropdown');
  const dropdownResultsEl = collabSelectorEl.querySelector('.collab-dropdown-results');
  const doneBtnEl = collabSelectorEl.querySelector('.collab-done-btn');

  let localSelectedCollaborators = buildCollaboratorEntries(slide.collaborators);
  let localHighlightedIndex = -1;
  let localDebounceTimer = null;

  function syncLocalSelection(shouldRenderPreview = true) {
    slide.collaborators = normalizeCollaboratorEmails(localSelectedCollaborators.map(entry => entry.email));

    if (!selectedChipsEl) return;
    if (localSelectedCollaborators.length === 0) {
      selectedChipsEl.innerHTML = '';
      selectedChipsEl.style.display = 'none';
    } else {
      selectedChipsEl.style.display = 'flex';
      selectedChipsEl.innerHTML = localSelectedCollaborators.map(entry => {
        const escaped = escapeCollabHtml(entry.title || entry.email);
        return `<span class="collab-selector-chip" data-email="${escapeCollabHtml(entry.email)}">
          <img src="${entry.avatar || DEFAULT_ICON}" alt="" class="collab-selector-chip-avatar">
          <span class="collab-selector-chip-name">${escaped}</span>
          <button type="button" class="collab-selector-chip-remove" aria-label="Remove">×</button>
        </span>`;
      }).join('');

      selectedChipsEl.querySelectorAll('.collab-selector-chip-remove').forEach(button => {
        button.addEventListener('click', (event) => {
          event.stopPropagation();
          const chip = button.closest('.collab-selector-chip');
          const email = chip?.getAttribute('data-email');
          if (!email) return;
          localSelectedCollaborators = localSelectedCollaborators.filter(entry => entry.email !== email);
          syncLocalSelection();
        });
      });
    }
  ...
  function renderLocalDropdown(searchTerm) {
    if (!dropdownResultsEl) return;
    const results = filterCollabSearchResults(searchTerm);
    const selectedEmails = new Set(localSelectedCollaborators.map(entry => entry.email));

    if (results.length === 0) {
      dropdownResultsEl.innerHTML = '<div class="collab-dropdown-empty">No matching users</div>';
      localHighlightedIndex = -1;
      return;
    }

    dropdownResultsEl.innerHTML = results.map((user, index) => {
      const isSelected = selectedEmails.has(user.email);
      const title = user.title || user.email.split('@')[0] || '?';
      return `<div class="collab-dropdown-row ${isSelected ? 'selected' : ''}" data-index="${index}" data-email="${escapeCollabHtml(user.email)}">
        <img src="${user.avatar || DEFAULT_ICON}" alt="" class="collab-dropdown-avatar">
        <span class="collab-dropdown-title">${escapeCollabHtml(title)}</span>
        <span class="collab-dropdown-check">${isSelected ? '✓' : ''}</span>
      </div>`;
    }).join('');

    localHighlightedIndex = -1;
    dropdownResultsEl.querySelectorAll('.collab-dropdown-row').forEach(row => {
      row.addEventListener('mousedown', event => event.preventDefault());
      row.addEventListener('click', () => {
        const email = row.getAttribute('data-email');
        const user = results.find(entry => entry.email === email);
        if (!user) return;
        const existingIndex = localSelectedCollaborators.findIndex(entry => entry.email === email);
        if (existingIndex >= 0) {
          localSelectedCollaborators.splice(existingIndex, 1);
        } else {
          localSelectedCollaborators.push({ email: user.email, title: user.title, avatar: user.avatar });
        }
        syncLocalSelection();
        if (searchInputEl) {
          searchInputEl.value = '';
  ...
  function showLocalDropdown() {
    if (dropdownEl) dropdownEl.classList.add('open');
    renderLocalDropdown(searchInputEl?.value || '');
    const rows = dropdownResultsEl?.querySelectorAll('.collab-dropdown-row');
    if (rows?.length) {
      localHighlightedIndex = 0;
      rows[0]?.classList.add('highlighted');
    }
  }

  function hideLocalDropdown() {
  ...
  if (searchInputEl) {
    searchInputEl.addEventListener('focus', () => {
      if (profileAllUsersCache.length === 0 && window.firebaseUtils?.getAllUsersArray) {
        window.firebaseUtils.getAllUsersArray().then(users => {
          profileAllUsersCache = users;
          localSelectedCollaborators = buildCollaboratorEntries(slide.collaborators);
          syncLocalSelection(false);
          showLocalDropdown();
        }).catch(() => {});
      } else {
        showLocalDropdown();
    ...
    searchInputEl.addEventListener('keydown', (event) => {
      ...
      const rows = dropdownResultsEl?.querySelectorAll('.collab-dropdown-row');
      if (!rows?.length) {
        if (event.key === 'Escape') hideLocalDropdown();
        return;
      }

      if (event.key === 'ArrowDown') {
        event.preventDefault();
        localHighlightedIndex = Math.min(localHighlightedIndex + 1, rows.length - 1);
        rows[localHighlightedIndex]?.scrollIntoView({ block: 'nearest' });
        rows.forEach((row, index) => row.classList.toggle('highlighted', index === localHighlightedIndex));
      } else if (event.key === 'ArrowUp') {
        event.preventDefault();
        localHighlightedIndex = Math.max(localHighlightedIndex - 1, 0);
        rows[localHighlightedIndex]?.scrollIntoView({ block: 'nearest' });
        rows.forEach((row, index) => row.classList.toggle('highlighted', index === localHighlightedIndex));
      } else if (event.key === 'Enter' && localHighlightedIndex >= 0 && rows[localHighlightedIndex]) {
        event.preventDefault();
        rows[localHighlightedIndex].click();
  ...
  if (doneBtnEl) {
    doneBtnEl.addEventListener('mousedown', event => event.preventDefault());
  ...
  collabSelectorEl.addEventListener('focusout', () => {
    setTimeout(() => {
      if (!collabSelectorEl.contains(document.activeElement)) {
        hideLocalDropdown();
      }
    }, 0);
  });
...
if (blockTitleInput && blockDescInput) {
  initBlockTitleField(blockTitleInput, blockTitleCount, () => {
    renderBlockLivePreview();
    updateSaveBlockButtonState();
  });
  initBlockDescField(blockDescInput, blockDescCount, () => renderBlockLivePreview());
}
...
async function fetchLinkMetadataAndFillForm(url, titleInput, descInput, imgInput, imagePreviewDiv, loadingIndicatorContainer) {
  ...
  try {
    ...
    if (metadata.title) applyBlockTitleValue(titleInput, metadata.title);
    if (metadata.description) applyBlockDescValue(descInput, metadata.description);
...
function renderBlockLivePreview() {
  const collabRaw = blockCollaboratorsInput ? blockCollaboratorsInput.value.trim() : '';
  const collabEmails = collabRaw ? normalizeCollaboratorEmails(collabRaw.split(',').map(s => s.trim())) : [];
  const previewCollaboratorEmails = currentBlockType === 'carousel'
    ? getCarouselCollaboratorEmailsFromSlides(carouselSlides)
    : collabEmails;
  const hasValidEmails = previewCollaboratorEmails.some(e => e.includes('@'));
  if (hasValidEmails && profileAllUsersCache.length === 0 && window.firebaseUtils?.getAllUsersArray) {
    window.firebaseUtils.getAllUsersArray().then(users => {
      profileAllUsersCache = users;
      renderBlockLivePreview();
    }).catch(() => {});
  }
  const collabHtml = renderCollaboratorsPreviewHtml(collabEmails, { pendingEmails: collabEmails, allUsers: profileAllUsersCache });
  ...
  if (currentBlockType === 'embed' && currentEmbed) {
    ...
    blockLivePreview.innerHTML = `<p class='preview-text'>Block Preview:</p><div class='block' style='flex-direction:column;align-items:stretch;'>${collabHtml}${iframeHtml}<div style='padding:8px;'><strong>${blockTitleInput.value}</strong><br><small>${blockDescInput.value}</small></div></div>`;
    ...
    if (currentBlockType === 'default') {
      blockLivePreview.innerHTML = `<p class='preview-text'>Block Preview:</p><div class='block'><div class='block-content' style='flex-direction:column;align-items:flex-start;'>${collabHtml}<div style='display:flex;align-items:center;'><img src='${imgSrc}' style='height:40px;width:40px;object-fit:cover;border-radius:4px;'><div class='block-text' style='margin-left:12px;'><strong>${blockTitleInput.value}</strong><small>${blockDescInput.value}</small></div></div></div></div>`;
    } else {
      blockLivePreview.innerHTML = `<p class='preview-text'>Block Preview:</p><div class='block' style='flex-direction:column;align-items:flex-start;'>${collabHtml}<img src='${imgSrc}' style='width:100%;height:230px;object-fit:cover;border-radius:8px 8px 0 0;'><div style='padding:8px 0 0 0;'><strong>${blockTitleInput.value}</strong><br><small>${blockDescInput.value}</small></div></div>`;
    }
  } else if (currentBlockType === 'carousel') {
    let slidesHTML = '';
    for (let i = 0; i < carouselSlides.length; i++) {
      const slide = carouselSlides[i];
      const slideCollabEmails = normalizeCollaboratorEmails(slide.collaborators);
      const slideCollabHtml = renderCollaboratorsPreviewHtml(slideCollabEmails, {
        pendingEmails: slideCollabEmails,
        allUsers: profileAllUsersCache,
        extraClass: 'carousel-slide-collaborators'
      });
      let imgSrc = '';
      if (slide.imgFile) {
...
function setBlockType(type) {
  ...
  if ((type === 'default' || type === 'large-image') && currentBlockType === 'carousel' && carouselSlides.length > 0) {
    ...
    selectedCollaborators = buildCollaboratorEntries(first.collaborators || []);
    if (blockCollaboratorsInput) {
      blockCollaboratorsInput.value = normalizeCollaboratorEmails(first.collaborators).join(', ');
    }
    renderCollabSelectedChips();
  }
    ...
    carouselSlides = [
      {
        ...
        collaborators: normalizeCollaboratorEmails(selectedCollaborators.map(entry => entry.email)),
        isPermanent: true
      },
      { title: '', desc: '', link: '', imgFile: null, icon: '', collaborators: [], isPermanent: true }
    ];
  ...
  const blockCollaboratorsSection = document.getElementById('block-collaborators-section');
  if (blockCollaboratorsSection) {
    blockCollaboratorsSection.style.display = (type === 'carousel') ? 'none' : '';
  }
...
[blockLinkInput, blockImgInput].filter(Boolean).forEach(input => {
...
if (blockCollaboratorsInput) blockCollaboratorsInput.addEventListener('input', renderBlockLivePreview);

if (collabSearchInput) initCollabSelector();
...
function createCarouselSlideField(slide, idx) {
  ...
  const imageInputId = `carousel-slide-img-${idx}`;
  ...
  div.innerHTML = `
    ...
    <input type='url' placeholder='Paste a link to get started...' value='${escapeCollabHtml(slide.link || '')}' class='input-feild carousel-slide-link'>
    ...
    <div class='carousel-slide-details' style='display: ${detailsVisible};'>
      ...
      <input type='text' placeholder='Slide Title' value='${escapeCollabHtml(slide.title || '')}' class='input-feild carousel-slide-title' maxlength="100">
      <span class="block-field-counter carousel-slide-title-count" style="display:none;"></span>
      <p class="input-feild-title-text" style="margin-top: 10px;">Description:</p>
      <textarea placeholder='Slide Description' class='input-feild carousel-slide-desc' rows="3" maxlength="400">${escapeCollabHtml(slide.desc || '')}</textarea>
      <span class="block-field-counter carousel-slide-desc-count" style="display:none;"></span>
      <div class="carousel-slide-collab-section">
        <p class="input-feild-title-text" style="margin-top: 10px;">Slide Collaborators:</p>
        <div class="collab-selector carousel-slide-collab-selector">
          <div class="collab-selected-chips"></div>
          <div class="collab-input-wrap">
            <input type="text" class="collab-search-input input-feild" placeholder="Add collaborators" autocomplete="off">
          </div>
          <div class="collab-dropdown">
            <div class="collab-dropdown-header">Collaborators</div>
            <div class="collab-dropdown-results"></div>
            <button type="button" class="collab-done-btn">Done</button>
          </div>
        </div>
      </div>
      <p class="input-feild-title-text" style="margin-top: 10px;">Upload Block Image:</p>
      <label class="file-upload-btn carousel-slide-upload-btn" for="${imageInputId}">
        <svg class="file-upload-icon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor">
          <path d="M19.35 10.04C18.67 6.59 15.64 4 12 4 9.11 4 6.6 5.64 5.35 8.04 2.34 8.36 0 10.91 0 14c0 3.31 2.69 6 6 6h13c2.76 0 5-2.24 5-5 0-2.64-2.05-4.78-4.65-4.96zM14 13v4h-4v-4H7l5-5 5 5h-3z"/>
        </svg>
        <span class="carousel-slide-upload-text">${slide.icon ? 'Change Image' : 'Choose Image'}</span>
      </label>
      <input type='file' accept='image/*' class='carousel-slide-img' id='${imageInputId}'>
  ...
  const descInput = detailsContainer.querySelector('.carousel-slide-desc');
  const imgInput = detailsContainer.querySelector('.carousel-slide-img');
  const uploadLabelText = detailsContainer.querySelector('.carousel-slide-upload-text');

  initBlockSlideTextFields(div, slide, () => {
    renderBlockLivePreview();
    updateSaveBlockButtonState();
  });
  if (titleInput) {
    titleInput.addEventListener('input', updateSaveBlockButtonState);
  }
  imgInput.addEventListener('change', e => {
    slide.imgFile = e.target.files[0];
    if (uploadLabelText) {
      uploadLabelText.textContent = e.target.files[0] ? e.target.files[0].name : (slide.icon ? 'Change Image' : 'Choose Image');
    }
    renderBlockLivePreview();
  });
  imgInput.dataset.icon = slide.icon || DEFAULT_ICON;
  initCarouselSlideCollabSelector(div, slide);

  const actionButton = div.querySelector('.remove-slide-btn');
  actionButton.addEventListener('click', function () {
    if (isPermanentSlide) {
      // Clear functionality for the first two slides
      carouselSlides[idx] = { title: '', desc: '', link: '', imgFile: null, icon: '', collaborators: [], isPermanent: true };
      renderCarouselSlidesFields();
      renderBlockLivePreview();
    } else {
...
addCarouselSlideBtn.addEventListener('click', () => {
  if (carouselSlides.length < 10) {
    carouselSlides.push({ title: '', desc: '', link: '', imgFile: null, icon: '', collaborators: [], isPermanent: false });
    renderCarouselSlidesFields();
    renderBlockLivePreview();
  }
});
...
function resetBlockModal() {
  ...
  if (blockCollaboratorsInput) blockCollaboratorsInput.value = '';
  if (collabSearchInput) collabSearchInput.value = '';
  renderCollabSelectedChips();
...
function hasNonEmptyBlockTitle(value) {
  return String(value ?? '').trim().length > 0;
}

function canEnableSaveBlockForAdd() {
  if (currentBlockType === 'carousel') {
    const linkInputs = document.querySelectorAll('#carousel-slides-fields .carousel-slide-link');
    const titleInputs = document.querySelectorAll('#carousel-slides-fields .carousel-slide-title');
    let hasValidUrl = false;
    let hasTitle = false;
    linkInputs.forEach((inp) => {
      if (isValidUrlForFlow(inp.value.trim())) hasValidUrl = true;
    });
    titleInputs.forEach((inp) => {
      if (hasNonEmptyBlockTitle(inp.value)) hasTitle = true;
    });
    return hasValidUrl && hasTitle;
  }
  const url = blockLinkInput?.value?.trim() || '';
  const title = blockTitleInput?.value ?? '';
  return isValidUrlForFlow(url) && hasNonEmptyBlockTitle(title);
}

function updateSaveBlockButtonState() {
  if (!saveBlockBtn) return;
  const isEditing = editingBlockIndex !== null;
  const enabled = isEditing || canEnableSaveBlockForAdd();
  saveBlockBtn.disabled = !enabled;
  if (enabled) {
    saveBlockBtn.removeAttribute('aria-disabled');
    saveBlockBtn.removeAttribute('title');
  } else {
    saveBlockBtn.setAttribute('aria-disabled', 'true');
    saveBlockBtn.title = SAVE_BLOCK_DISABLED_TITLE;
  }
}
...
document.getElementById("save-block").addEventListener("click", async () => {
  ...
  if (saveBlockBtn?.disabled) return;
  ...
  try {
    const email = window.firebaseUtils.currentUser.email;
    const userId = window.firebaseUtils.requireUserId();
    const authUid = window.firebaseUtils.requireAuthUid();
    ...
    // Parse collaborators. Carousel blocks store them per slide, but keep a block-level union for request logic.
    const collaboratorsRaw = blockCollaboratorsInput ? blockCollaboratorsInput.value.trim() : '';
    const newCollaborators = currentBlockType === 'carousel'
      ? getCarouselCollaboratorEmailsFromSlides(carouselSlides)
      : normalizeCollaboratorEmails(
          collaboratorsRaw
            ? collaboratorsRaw.split(',').map(s => s.trim())
            : []
        );

    // When editing: remove globalId from collaborators who were removed
    if (editingBlockIndex !== null && existingGlobalId) {
      const oldCollaborators = getBlockCollaboratorEmails(blocks[editingBlockIndex]);
      const removed = oldCollaborators.filter(o => !newCollaborators.includes(o));
      for (const removedEmail of removed) {
        await window.firebaseUtils.removeCollaborationFromUser(removedEmail, existingGlobalId);
      }
    }
    ...
    if (currentBlockType === 'embed' && currentEmbed) {
      block = {
        ...
        title: truncateBlockTitle(blockTitleInput.value),
        desc: truncateBlockDesc(blockDescInput.value),
      ...
      if (file) {
        ...
        imgUrl = await window.firebaseUtils.uploadImage(
          compressedFile,
          window.firebaseUtils.blockStoragePath(authUid, Date.now())
        );
      }
      let icon = imgUrl;
      if (!icon) {
      ...
      for (let i = 0; i < carouselSlides.length; i++) {
        ...
        if (carouselSlides[i].imgFile) {
          ...
          imgUrl = await window.firebaseUtils.uploadImage(
            compressedFile,
            window.firebaseUtils.blockCarouselStoragePath(authUid, i)
          );
        }
        slides.push({
          title: truncateBlockTitle(carouselSlides[i].title),
          desc: truncateBlockDesc(carouselSlides[i].desc),
          link: normalizeUrl(carouselSlides[i].link),
          icon: imgUrl || carouselSlides[i].icon || DEFAULT_ICON,
          collaborators: normalizeCollaboratorEmails(carouselSlides[i].collaborators)
        });
      }
      block = {
        type: 'carousel',
    ...
    if (isBlockEmptyForGlobalPublish(block)) {
      if (existingGlobalId) {
        await window.firebaseUtils.deleteGlobalBlock(existingGlobalId);
        delete block.globalId;
        await window.firebaseUtils.saveUserData(email, { blocks });
      }
    } else {
      const globalId = await window.firebaseUtils.upsertGlobalBlock(email, block, userId);
      if (globalId) {
...
function renderBlocks(arr, allUsersForCollab = null) {
  blocks = arr;
  blocksContainer.innerHTML = "";

  const hasBlocksWithCollab = arr.some(b => getBlockCollaboratorEmails(b).length > 0);
  if (hasBlocksWithCollab && !allUsersForCollab && window.firebaseUtils?.getAllUsersArray) {
    window.firebaseUtils.getAllUsersArray().then(users => {
      profileAllUsersCache = users;
      renderBlocks(arr, users);
    }).catch(() => {});
  } else if (allUsersForCollab) {
    profileAllUsersCache = allUsersForCollab;
  }

  arr.forEach((block, index) => {
    const collab = getBlockCollaboratorEmails(block);
    const pendingEmails = getPendingCollaboratorEmails(collab, block.globalId, allUsersForCollab || []);
    const collabHtml = renderCollaboratorsPreviewHtml(collab, { pendingEmails, allUsers: allUsersForCollab || [] });
    let blockHTML = '';
    let actionsHTML = `
    ...
    if (block.type === 'embed') {
      ...
      blockHTML = `<div style='flex-direction:column;align-items:stretch;display:flex;width:100%;'>
        ${collabHtml}
        <div style="display:flex;align-items:center;padding-left: 10px; padding-top: 10px; padding-right: 10px;">
      ...
      blockHTML = `
        <div class="block-content" style="flex-direction:column;align-items:flex-start;display:flex;width:100%;">
          ${collabHtml}
          <div style="display:flex;align-items:center; padding-left: 10px; padding-top: 10px;">
            <div class="drag-handle" style="cursor:move;padding:8px;color:#666;display:flex;align-items:center;justify-content:center;border-radius:4px;background:#f0f0f0;margin-right:8px;position:relative;" draggable="true" data-index="${index}" title="Drag to reorder">
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                <path d="M8 9h8M8 15h8"/>
              </svg>
      ...
      blockHTML = `<div style='flex-direction:column;align-items:flex-start;display:flex;width:100%;'>
        ${collabHtml}
        <div style="display:flex;align-items:center; padding-right: 10px; padding-left: 10px; padding-top: 10px;">
          <div class="drag-handle" style="cursor:move;padding:8px;color:#666;display:flex;align-items:center;justify-content:center;border-radius:4px;background:#f0f0f0;margin-right:8px;position:relative;" draggable="true" data-index="${index}" title="Drag to reorder">
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
              <path d="M8 9h8M8 15h8"/>
            </svg>
          </div>
          <img src="${block.icon || DEFAULT_ICON}" style="width:100%;height:230px;object-fit:cover;border-radius:8px 8px 0 0; overflow:hidden;">
        </div>
        <div style='padding:8px 0 0 0; margin-left:46px;'><strong>${block.title}</strong><br><small>${block.desc}</small></div>${actionsHTML}</div>`;
    } else if (block.type === 'carousel') {
      let slidesHTML = '';
      for (let i = 0; i < block.slides.length; i++) {
        const slide = block.slides[i];
        const slideCollab = normalizeCollaboratorEmails(slide.collaborators);
        const slidePendingEmails = getPendingCollaboratorEmails(slideCollab, block.globalId, allUsersForCollab || []);
        const slideCollabHtml = renderCollaboratorsPreviewHtml(slideCollab, {
          pendingEmails: slidePendingEmails,
          allUsers: allUsersForCollab || [],
          extraClass: 'carousel-slide-collaborators'
        });
        slidesHTML += `<div style='display:inline-flex;flex-direction:column;width:230px;margin-right:8px;vertical-align:top;'>${slideCollabHtml}<div style='background:#fff;border-radius:8px;box-shadow:0 2px 8px #0001;overflow:hidden;'><img src='${slide.icon || DEFAULT_ICON}' style='width:230px;height:230px;object-fit:cover;display:block;'><div style='padding:8px;'><strong>${slide.title || ''}</strong><br><small>${slide.desc || ''}</small></div></div></div>`;
      }
      blockHTML = `<div style='flex-direction:column;align-items:stretch;display:flex;width:100%;'>
        <div style="display:flex;align-items:center; padding-left: 10px; padding-top: 10px;">
          <div class="drag-handle" style="cursor:move;padding:8px;color:#666;display:flex;align-items:center;justify-content:center;border-radius:4px;background:#f0f0f0;margin-right:8px;position:relative;" draggable="true" data-index="${index}" title="Drag to reorder">
...
window.editBlock = function (index) {
  ...
  const blockCollaboratorEmails = getBlockCollaboratorEmails(block);
  if (blockCollaboratorEmails.length > 0 && profileAllUsersCache.length > 0) {
    selectedCollaborators = buildCollaboratorEntries(blockCollaboratorEmails);
  } else if (blockCollaboratorEmails.length > 0) {
    selectedCollaborators = blockCollaboratorEmails.map(email => ({
      email,
      title: email.split('@')[0],
      avatar: DEFAULT_ICON
    }));
  ...
  if (blockCollaboratorsInput) blockCollaboratorsInput.value = selectedCollaborators.map(c => c.email).join(', ');
  renderCollabSelectedChips();
  if (profileAllUsersCache.length === 0 && window.firebaseUtils?.getAllUsersArray) {
    window.firebaseUtils.getAllUsersArray().then(users => {
      profileAllUsersCache = users;
      selectedCollaborators = blockCollaboratorEmails.map(email => {
        ...
        return u ? { email: u.email, title: u.title, avatar: u.avatar } : { email, title: email.split('@')[0], avatar: DEFAULT_ICON };
      });
      renderCollabSelectedChips();
    }).catch(() => {});
  }
  if (block.type === 'embed') {
    ...
    carouselSlides = block.slides.map((slide, i) => ({
      ...
      collaborators: normalizeCollaboratorEmails(slide.collaborators),
      isPermanent: i < 2
    }));
    renderCarouselSlidesFields();
  }
...
window.deleteBlock = async function (index) {
  ...
  if (confirm('Are you sure you want to delete this block?')) {
    ...
    try {
      ...
      if (!globalId && window.firebaseUtils.resolveGlobalBlockIdForDelete) {
        globalId = window.firebaseUtils.resolveGlobalBlockIdForDelete(blockToDelete, {
          userId: window.firebaseUtils.currentUserDoc?.id,
          ownerEmail,
        });
      }
  ...
  if (previewBtn && previewModal && previewModalClose && previewIframe) {
    previewBtn.addEventListener('click', () => {
      ...
      if (userDoc && userDoc.id) {
        ...
        const previewUrl = buildProfileUrl(title);
