...
function renderCollaboratorsPreviewHtml(emails, options = {}) {
  const { pendingEmails = [], allUsers = [] } = options;
...
function renderBlockLivePreview() {
  ...
  const collabEmails = collabRaw ? collabRaw.split(',').map(s => s.trim()).filter(Boolean) : [];
  const hasValidEmails = collabEmails.some(e => e.includes('@'));
  ...
  if (currentBlockType === 'embed' && currentEmbed) {
    ...
    blockLivePreview.innerHTML = `<p class='preview-text'>Block Preview:</p><div class='block' style='flex-direction:column;align-items:stretch;max-width:350px;'>${collabHtml}${iframeHtml}<div style='padding:8px 0 0 0;'><strong>${blockTitleInput.value}</strong><br><small>${blockDescInput.value}</small></div></div>`;
    ...
    if (currentBlockType === 'default') {
      blockLivePreview.innerHTML = `<p class='preview-text'>Block Preview:</p><div class='block' style='max-width:350px;'><div class='block-content' style='flex-direction:column;align-items:flex-start;'>${collabHtml}<div style='display:flex;align-items:center;'><img src='${imgSrc}' style='height:40px;width:40px;object-fit:cover;border-radius:4px;'><div class='block-text' style='margin-left:12px;'><strong>${blockTitleInput.value}</strong><small>${blockDescInput.value}</small></div></div></div></div>`;
    } else {
      blockLivePreview.innerHTML = `<p class='preview-text'>Block Preview:</p><div class='block' style='flex-direction:column;align-items:flex-start;max-width:350px;'>${collabHtml}<img src='${imgSrc}' style='width:100%;height:230px;object-fit:cover;border-radius:8px 8px 0 0;'><div style='padding:8px 0 0 0;'><strong>${blockTitleInput.value}</strong><br><small>${blockDescInput.value}</small></div></div>`;
    }
    ...
    blockLivePreview.innerHTML = `<p class='preview-text'>Block Preview:</p>${collabHtml}<div id="preview-carousel-container" style='overflow-x:auto;white-space:nowrap;max-width:100%;'>${slidesHTML}</div>`;
...
document.getElementById("save-block").addEventListener("click", async () => {
  ...
  try {
    ...
    // Parse collaborators (comma-separated, trimmed)
    const collaboratorsRaw = blockCollaboratorsInput ? blockCollaboratorsInput.value.trim() : '';
    const newCollaborators = collaboratorsRaw
    ...
    if (editingBlockIndex !== null && existingGlobalId) {
      const oldCollaborators = Array.isArray(blocks[editingBlockIndex].collaborators) ? blocks[editingBlockIndex].collaborators : [];
...
function renderBlocks(arr, allUsersForCollab = null) {
  ...
  const hasBlocksWithCollab = arr.some(b => Array.isArray(b.collaborators) && b.collaborators.length > 0);
  ...
  arr.forEach((block, index) => {
    ...
    const pendingEmails = allUsersForCollab && block.globalId
      ? collab.filter(email => {
          const u = allUsersForCollab.find(x => x.email === email);
          return !u || !(Array.isArray(u.collaborations) ? u.collaborations : []).includes(block.globalId);
        })
      : collab;
...
window.editBlock = function (index) {
  ...
  if (block.collaborators && profileAllUsersCache.length > 0) {
    selectedCollaborators = (block.collaborators || [])
      .map(email => {
        const u = profileAllUsersCache.find(x => x.email === email);
        return u ? { email: u.email, title: u.title, avatar: u.avatar } : { email, title: email.split('@')[0], avatar: DEFAULT_ICON };
      })
      .filter(Boolean);
  } else if (block.collaborators) {
    selectedCollaborators = (block.collaborators || []).map(email => ({
      email,
      title: email.split('@')[0],
      avatar: DEFAULT_ICON
    }));
